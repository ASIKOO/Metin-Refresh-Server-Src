...
typedef struct SSimplePlayer
{
	...
	DWORD		titre; // système de titre
	DWORD		cheque; // système de won
} TSimplePlayer;
...
typedef struct SPlayerTable
{
	...
	int		Semeskill; // 7ème skill
	int		Eemeskill; // 8ème skill
	int		aiPremiumTimes[PREMIUM_MAX_NUM];
	long long	cheque;
	int		titre; // Système de titre
} TPlayerTable;
...
typedef struct SMobTable : public SEntityTable
{
	...
	BYTE	bRevivePoint;
	//char	bScale[CHARACTER_NAME_MAX_LEN + 1];
} TMobTable;
...
typedef struct SShopItemTable
{
	...
	long long		price;	// PC, shop_table_ex.txt »óÁ¡¿¡¸¸ ÀÌ¿ë
