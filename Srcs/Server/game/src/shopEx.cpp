...
long long CShopEx::Buy(LPCHARACTER ch, BYTE pos)
{
	...
	case SHOP_COIN_TYPE_GOLD:
		...
		{
			sys_log(1, "ShopEx::Buy : Not enough money : %s has %ll, price %ll", ch->GetName(), ch->GetGold(), dwPrice);
			return SHOP_SUBHEADER_GC_NOT_ENOUGH_MONEY;
		}
		break;
	case SHOP_COIN_TYPE_SECONDARY_COIN:
		{
			...
			if (count < dwPrice)
			{
				sys_log(1, "ShopEx::Buy : Not enough myeongdojun : %s has %ll, price %ll", ch->GetName(), count, dwPrice);
	...
	if (item)
		sys_log(0, "ShopEx: BUY: name %s %s(x %d):%u price %ll", ch->GetName(), item->GetName(), item->GetCount(), item->GetID(), dwPrice);
