...
void CHARACTER::__StateIdle_Monster()
{
	...
	if (victim != NULL)
	{
		if (IsMonster() && victim->IsPC())
		{
			if (victim->m_nouveauMouvementSystem != NULL)
			{
				if (victim->m_nouveauMouvementSystem->IsNage || victim->m_nouveauMouvementSystem->IsTombe)
				{
					return;
				}
				else
				{
					if (victim->m_nouveauMouvementSystem->IsVol)
					{
						if (victim->m_nouveauMouvementSystem->hauteurZ - victim->m_nouveauMouvementSystem->hauteurSol > 200)
						{
							return;
						}
					}
...
void CHARACTER::StateMove()
{
	...
	else
	{
		...
		{
			if (IsMonster() && GetVictim()->IsPC())
			{
				if (GetVictim()->m_nouveauMouvementSystem != NULL)
				{
					if (GetVictim()->m_nouveauMouvementSystem->IsNage || GetVictim()->m_nouveauMouvementSystem->IsTombe)
					{
						return;
					}
					else
					{
						if (GetVictim()->m_nouveauMouvementSystem->IsVol)
						{
							if (GetVictim()->m_nouveauMouvementSystem->hauteurZ - GetVictim()->m_nouveauMouvementSystem->hauteurSol > 200)
							{
								return;
							}
						}
