...
void CHARACTER::Initialize()
{
	...
	HerraShopEvent = NULL;
	m_itemBloquerSystem = 0;
	VnumShop = 0;

	m_bOpeningSafebox = false;
	PlayerIsDuel = false;
	m_invincibleStatut = 0;
	...
	TestEnchantementPlus = false;
	TypeBonusEnchantement0 = 0;
	ValeurBonusEnchantement0 = 0;
	TypeBonusEnchantement1 = 0;
	ValeurBonusEnchantement1 = 0;
	TypeBonusEnchantement2 = 0;
	ValeurBonusEnchantement2 = 0;
	TypeBonusEnchantement3 = 0;
	ValeurBonusEnchantement3 = 0;
	TypeBonusEnchantement4 = 0;
	ValeurBonusEnchantement4 = 0;
#endif
	m_offlineShopSystem = 0;
	m_CadeauSystem = 0;
	m_titreSystem = 0;
	m_charInventaireSystem = 0;
#ifdef ENABLE_ENVIRONNEMENT_SYSTEM
	m_environnementSystem = 0;
#endif
#ifdef ENABLE_METEO_SYSTEM
	m_meteoSystem = 0;
#endif
	m_magasinierSystem = 0;
	m_antiCheatSystem = 0;
	m_MobScaleSystem = 0;
	m_nouveauMouvementSystem = 0;
	m_fAttMul = 1.0f;
	m_fDamMul = 1.0f;

	m_invincibleStatut = false;
...
//		BindDesc(NULL);
	...
	if (m_offlineShopSystem)
	{
		M2_DELETE(m_offlineShopSystem);
		m_offlineShopSystem = NULL;
	}
	if (m_magasinierSystem)
	{
		M2_DELETE(m_magasinierSystem);
		m_magasinierSystem = NULL;
	}
	if (m_antiCheatSystem)
	{
		M2_DELETE(m_antiCheatSystem);
		m_antiCheatSystem = NULL;
	}
	if (m_MobScaleSystem)
	{
		M2_DELETE(m_MobScaleSystem);
		m_MobScaleSystem = NULL;
	}
	if (m_nouveauMouvementSystem)
	{
		M2_DELETE(m_nouveauMouvementSystem);
		m_nouveauMouvementSystem = NULL;
	}

	if (m_CadeauSystem)
	{
		M2_DELETE(m_CadeauSystem);
		m_CadeauSystem = NULL;
	}

	if (m_titreSystem)
	{
		m_titreSystem->RetraitBonusTitre(this);
		m_titreSystem->ArretUpdateTitre();
		M2_DELETE(m_titreSystem);
		m_titreSystem = NULL;
	}
	if (m_charInventaireSystem)
	{
		M2_DELETE(m_charInventaireSystem);
		m_charInventaireSystem = NULL;
	}

	if (m_itemBloquerSystem)
	{
		if (&m_itemBloquerSystem->m_pkCheckItem != NULL)
		{
			event_cancel(&m_itemBloquerSystem->m_pkCheckItem);
		}
		M2_DELETE(m_itemBloquerSystem);
		m_itemBloquerSystem = NULL;
	}

#ifdef ENABLE_ENVIRONNEMENT_SYSTEM
	if (m_environnementSystem)
	{
		if (&m_environnementSystem->m_pkEnvironnementSystemeUpdateEvent != NULL)
		{
			event_cancel(&m_environnementSystem->m_pkEnvironnementSystemeUpdateEvent);
		}
		M2_DELETE(m_environnementSystem);
		m_environnementSystem = NULL;
	}
#endif
#ifdef ENABLE_METEO_SYSTEM
	if (m_meteoSystem)
	{
		if (&m_meteoSystem->m_pkMeteoSystemeUpdateEvent != NULL)
		{
			event_cancel(&m_meteoSystem->m_pkMeteoSystemeUpdateEvent);
		}
		M2_DELETE(m_meteoSystem);
		m_meteoSystem = NULL;
	}
#endif
...
EVENTFUNC(Herra_shops_event)
{
	Herra_shop_event* info = dynamic_cast<Herra_shop_event*>( event->info );
	if (info == NULL)
	{
		sys_err("Fucking error");
		return 0;
	}
	
	LPCHARACTER ch = CHARACTER_MANAGER::instance().FindByPID(info->owner_id);
	LPCHARACTER npc = CHARACTER_MANAGER::instance().Find(info->shop_npc);
	
	if (ch && npc)
	{
		DBManager::instance().DirectQuery("UPDATE private_herra_shops%s SET status = '2' WHERE shop_id = '%d' AND owner_id = '%d';", get_table_postfix(), info->shop_npc, info->owner_id);
		ch->ChatPacket(CHAT_TYPE_INFO, "Votre magasin privé a été fermé.");
		ch->ChatPacket(CHAT_TYPE_INFO, "Pour voir ce qui a été vendu veuillez ouvrir le panneau de gestion de magasin.");
		npc->CloseHerraShop();
	}
	else if (npc)
	{
...
void CHARACTER::CreateHerraShopEvent(int secTime, int owner_id, int npc_vid)
{
	Herra_shop_event* info = AllocEventInfo<Herra_shop_event>();
	info->owner_id = owner_id;
	info->shop_npc = npc_vid;
	info->temps = secTime;
 
	HerraShopEvent = event_create(Herra_shops_event, info, PASSES_PER_SEC(secTime));
}


void CHARACTER::OpenMyShop(const char * c_pszSign, TShopItemTable2 * pTable, int bItemCount, int days)
{
	// Test
	if (bItemCount == 0)
		return;
	...
	{
		ChatPacket(CHAT_TYPE_INFO, "<GM>: Les membres du staff ne peuvent pas ouvrir de magasin.");	
		return;		
	}
	
	char szQuery[1024+1];
	snprintf(szQuery, sizeof(szQuery), "SELECT * FROM private_herra_shops%s WHERE status = '1' AND owner_id = '%d';", get_table_postfix(), GetPlayerID());
	std::unique_ptr<SQLMsg> msg(DBManager::instance().DirectQuery(szQuery));
	int shopAmount = msg->Get()->uiNumRows;
	
	if (shopAmount > 0)
	{
		ChatPacket(CHAT_TYPE_INFO, "Vous ne pouvez avoir qu'un seul magasin privé.");	
		return;
	}

	std::string tempSign = (std::string) c_pszSign;
	int tailleNomShop = tempSign.size();

	if (tailleNomShop > (SHOP_SIGN_MAX_LEN))
	{
		ChatPacket(CHAT_TYPE_INFO, "Le nom de votre magasin est trop long.");
		return;
	}

	if (tempSign.find("|") != std::string::npos)
	{
		ChatPacket(CHAT_TYPE_INFO, "Le caractère spéciaux | est interdit dans le nom de magasin.");
		return;
	}

	if (CBanwordManager::instance().CheckString(tempSign.c_str(), tempSign.length()))
	{
		ChatPacket(CHAT_TYPE_INFO, "Le nom de votre magasin est invalide.");
		return;
	}
	...
	if(bItemCount <= 0)
	{
		ChatPacket(CHAT_TYPE_COMMAND, "TitreMessage Vous-devez-ajouter-au-moins-1-objet-dans-votre-magasin.");	
		return;
	}
	...
	if (CountSpecifyItem(50200)) // Paquet
	{
		RemoveSpecifyItem(50200, 1);
	}
	else if (CountSpecifyItem(71221)) // Paquet de cachemire
	{
		...
		for (int i = 0; i < inventorysize; ++i)
		{
			...
			if (NULL != item && item->GetVnum() == 71221)
			{
				if (item->GetLimitValue(0) <= 0)
				{
					RemoveSpecifyItem(71221, 1);
				}
			}
		}
	}
	...
	int randomX = number(1, 200);
	int randomY = number(1, 200);

	if (VnumShop <= 0)
	{
		VnumShop = 30000;
	}
	LPCHARACTER npcShop = CHARACTER_MANAGER::instance().SpawnMob(VnumShop, GetMapIndex(), GetX(), GetY(), GetZ(), false, -1, false);
	
	if (!npcShop)
		return;
		
	char shopName[1024];
	snprintf(shopName, sizeof(shopName), "%s - Magasin", GetName());
		
	npcShop->SetName(shopName);
	npcShop->Show(GetMapIndex(), GetX()+randomX, GetY()+randomY, 0, false);

	TPacketGCShopSign p;

	p.bHeader = HEADER_GC_SHOP_SIGN;
	p.dwVID = npcShop->GetVID();
	strlcpy(p.szSign, c_pszSign, sizeof(p.szSign));

	PacketAround(&p, sizeof(TPacketGCShopSign));

	npcShop->HerraShop = CShopManager::instance().CreatePCShop(npcShop, this, c_pszSign, pTable, bItemCount, GetMapIndex(), GetX()+randomX, GetY()+randomY, days, VnumShop);
	npcShop->HerraShopVID = npcShop->GetVID();
	npcShop->m_stShopSign = m_stShopSign;

	int shopVID = npcShop->GetVID();

}

void CHARACTER::RespawnMyShop(LPCHARACTER npcShop, const char * title, LPSHOP shop)
{
	...
	p.bHeader = HEADER_GC_SHOP_SIGN;
	p.dwVID = npcShop->GetVID();
	strlcpy(p.szSign, title, sizeof(p.szSign));

	PacketAround(&p, sizeof(TPacketGCShopSign));

	npcShop->HerraShop = shop;
	npcShop->HerraShopVID = npcShop->GetVID();
	npcShop->m_stShopSign = title;

}

void CHARACTER::CloseHerraShop()
{
	if (GetHerraShop())
	{
		
		LPCHARACTER shopNPC = CHARACTER_MANAGER::instance().Find(GetHerraShopVID());
		
		m_stShopSign.clear();
		CShopManager::instance().DestroyPCShop(shopNPC);
		HerraShop = NULL;
		HerraShopVID = 0;
		...
		p.bHeader = HEADER_GC_SHOP_SIGN;
		p.dwVID = GetHerraShopVID();
		p.szSign[0] = '\0';

		PacketAround(&p, sizeof(p));
		
		M2_DESTROY_CHARACTER(shopNPC);
...
void CHARACTER::EncodeInsertPacket(LPENTITY entity)
{
	...
	if (IsPC())
	{
		if (SYSTEM_TITRE > 0)
		{
			if (m_titreSystem != NULL)
			{
				if (m_titreSystem->niveauTitreTueur <= 0)
				{
					m_titreSystem->niveauTitreTueur = m_points.title;
				}
				pack.dwTitle = m_points.title;
				m_titreSystem->GetPacketTitre(this, GetName(), m_titreSystem->niveauTitreTueur);
				

				size_t destination_size = sizeof (pack.dwTitreNom);

				strncpy(pack.dwTitreNom, m_titreSystem->nomTitre.c_str(), destination_size);
				strncpy(pack.dwTitreCouleur1, m_titreSystem->couleur1Titre.c_str(), destination_size);
				strncpy(pack.dwTitreCouleur2, m_titreSystem->couleur2Titre.c_str(), destination_size);
				strncpy(pack.dwTitreCouleur3, m_titreSystem->couleur3Titre.c_str(), destination_size);
			}
		}
		else
		{
			size_t destination_size = sizeof (pack.dwTitreNom);
			strncpy(pack.dwTitreNom, "", destination_size);
			strncpy(pack.dwTitreCouleur1, "", destination_size);
			strncpy(pack.dwTitreCouleur2, "", destination_size);
			strncpy(pack.dwTitreCouleur3, "", destination_size);
		}
	}
	*/
		...
		if (IsPC())
		{
			if (SYSTEM_TITRE > 0)
			{
				if (m_titreSystem != NULL)
				{
					...
					addPacket.dwTitle = m_points.title;
					if (m_titreSystem->niveauTitreTueur <= 0)
					{
						m_titreSystem->niveauTitreTueur = m_points.title;
					}
					m_titreSystem->GetPacketTitre(this, GetName(), m_titreSystem->niveauTitreTueur);
					size_t destination_size = sizeof (pack.dwTitreNom);
					strncpy(addPacket.dwTitreNom, m_titreSystem->nomTitre.c_str(), destination_size);
					strncpy(addPacket.dwTitreCouleur1, m_titreSystem->couleur1Titre.c_str(), destination_size);
					strncpy(addPacket.dwTitreCouleur2, m_titreSystem->couleur2Titre.c_str(), destination_size);
					strncpy(addPacket.dwTitreCouleur3, m_titreSystem->couleur3Titre.c_str(), destination_size);

				}
			}
			else
			{
				size_t destination_size = sizeof (addPacket.dwTitreNom);
				strncpy(addPacket.dwTitreNom, "", destination_size);
				strncpy(addPacket.dwTitreCouleur1, "", destination_size);
				strncpy(addPacket.dwTitreCouleur2, "", destination_size);
				strncpy(addPacket.dwTitreCouleur3, "", destination_size);
			}
		}
		*/
		if (SYSTEM_NOUVEAU_SKILL_7ET8 > 0 && IsPC())
		{
			addPacket.dw7emeSkill = m_points.NewFirst_skill;
			addPacket.dw8emeSkill = m_points.NewSecond_skill;
		}
#ifdef NEW_PET_SYSTEM
...
void CHARACTER::UpdatePacket()
{
	...
	if (IsPC())
	{
		ChatPacket(CHAT_TYPE_COMMAND, "Won %d", GetCheque());
	}
#endif
...
/*
	if (SYSTEM_TITRE > 0)
	{
		if (m_titreSystem != NULL)
		{
			pack.bTitle = m_points.title;
			if (m_titreSystem->AFK == false)
			{
				if (m_titreSystem->niveauTitreTueur <= 0)
				{
					m_titreSystem->niveauTitreTueur = m_points.title;
				...
				size_t destination_size = sizeof (pack.bTitreNom);

				if (m_titreSystem->titre_vector_joueur.size() > 0)
				{
					strncpy(pack.bTitreNom, m_titreSystem->nomTitre.c_str(), destination_size);
					strncpy(pack.bTitreCouleur1, m_titreSystem->couleur1Titre.c_str(), destination_size);
					strncpy(pack.bTitreCouleur2, m_titreSystem->couleur2Titre.c_str(), destination_size);
					strncpy(pack.bTitreCouleur3, m_titreSystem->couleur3Titre.c_str(), destination_size);
				}
				else
				{
					strncpy(pack.bTitreNom, "", destination_size);
					strncpy(pack.bTitreCouleur1, m_titreSystem->couleur1Titre.c_str(), destination_size);
					strncpy(pack.bTitreCouleur2, m_titreSystem->couleur2Titre.c_str(), destination_size);
					strncpy(pack.bTitreCouleur3, m_titreSystem->couleur3Titre.c_str(), destination_size);
				}
			...
			else
			{
				...
				strncpy(pack.bTitreNom, "AFK", destination_size);
				strncpy(pack.bTitreCouleur1, m_titreSystem->couleur1Titre.c_str(), destination_size);
				strncpy(pack.bTitreCouleur2, m_titreSystem->couleur2Titre.c_str(), destination_size);
				strncpy(pack.bTitreCouleur3, m_titreSystem->couleur3Titre.c_str(), destination_size);
			}
...
*/
	if (SYSTEM_NOUVEAU_SKILL_7ET8 > 0)
	{
		pack.b7emeSkill = m_points.NewFirst_skill;
		pack.b8emeSkill = m_points.NewSecond_skill;
	}
	...
	pack2.sAlignment = 0;
#ifdef NEW_PET_SYSTEM
	pack2.dwLevel = GetLevel();
#endif


	if (SYSTEM_NOUVEAU_SKILL_7ET8 > 0)
	{
		pack2.b7emeSkill = m_points.NewFirst_skill;
		pack2.b8emeSkill = m_points.NewSecond_skill;
	}
	if (false)
	{
...
void CHARACTER::Get7emeSkillLogin()
{
	std::unique_ptr<SQLMsg> msg(DBManager::instance().DirectQuery("SELECT 7emeSkill FROM player%s WHERE name='%s';", get_table_postfix(), GetName()));
	MYSQL_ROW row = mysql_fetch_row(msg->Get()->pSQLResult);
		
	int idSkill = 0;
	str_to_number(idSkill, row[0]);
	PointChange(POINT_7EMESKILL, idSkill, false, true);
	UpdatePacket();
	SetNewSkill(idSkill); 
	ChatPacket(CHAT_TYPE_COMMAND, "7emeSkill %d", idSkill);
}

void CHARACTER::Get8emeSkillLogin()
{
	std::unique_ptr<SQLMsg> msg(DBManager::instance().DirectQuery("SELECT 8emeSkill FROM player%s WHERE name='%s';", get_table_postfix(), GetName()));
	MYSQL_ROW row = mysql_fetch_row(msg->Get()->pSQLResult);
		
	int idSkill = 0;
	str_to_number(idSkill, row[0]);
	PointChange(POINT_8EMESKILL, idSkill, false, true);
	UpdatePacket();
	SetNewerSkill(idSkill);
	ChatPacket(CHAT_TYPE_COMMAND, "8emeSkill %d", idSkill);
}
...
void CHARACTER::CreatePlayerProto(TPlayerTable & tab)
{
	...
	tab.Semeskill = m_points.NewFirst_skill; // 7ème skill
	tab.Eemeskill = m_points.NewSecond_skill; // 8ème skill	
	if (m_titreSystem != NULL)
	{
		if (m_titreSystem->niveauTitreTueur > 0)
		{
			tab.titre = m_titreSystem->niveauTitreTueur; // Titre
		}
		else
		{
			SQLMsg * pMsg3 = DBManager::instance().DirectQuery("select titre from player%s WHERE name='%s'", get_table_postfix(), GetName());
			if (pMsg3->Get()->uiNumRows > 0)
			{
				MYSQL_ROW row = mysql_fetch_row(pMsg3->Get()->pSQLResult);
				int tempTitre = 0;
				str_to_number(tempTitre, row[0]);
				tab.titre = tempTitre;
			}
		}
	}
...
void CHARACTER::Disconnect(const char * c_pszReason)
{
	...
	std::unique_ptr<SQLMsg> pmsgUpdateCompte (DBManager::instance().DirectQuery("UPDATE account_alpha.account SET connect=0 WHERE id='%d'", GetAID()));
...
void CHARACTER::SetTitle(int level)
{
	SetPoint(POINT_TITLE, level);
	m_points.title = level;
}
...
void CHARACTER::SetNewSkill(int idSkill)
{
	SetPoint(POINT_7EMESKILL, idSkill);
	m_points.NewFirst_skill = idSkill;
}

void CHARACTER::SetNewerSkill(int idSkill)
{
	SetPoint(POINT_8EMESKILL, idSkill);
	m_points.NewSecond_skill = idSkill;
}
...
void CHARACTER::SetPlayerProto(const TPlayerTable * t)
{
...
#ifdef ENABLE_CHEQUE_SYSTEM
	SetCheque(t->cheque);
	SetPoint(POINT_CHEQUE, t->cheque);
	m_points.cheque = t->cheque;
#endif
	...
	if (SYSTEM_TITRE > 0)
	{
		SetPoint(POINT_TITLE, t->titre);
		SetTitle(t->titre); // Titre
		m_points.title = t->titre;
	}

	if (SYSTEM_NOUVEAU_SKILL_7ET8 > 0)
	{
		SetNewSkill(t->Semeskill); // Set 7ème skill
		SetNewerSkill(t->Eemeskill); // Set 8ème skill
		SetPoint(POINT_7EMESKILL, t->Semeskill);
		SetPoint(POINT_8EMESKILL, t->Eemeskill);
		m_points.NewFirst_skill = t->Semeskill;
		m_points.NewSecond_skill = t->Eemeskill;
		sys_log(0, "PLAYER_OPTION_Metin2France: 7ème SKILL: %d , 8ème SKILL: %d", t->Semeskill, t->Eemeskill);
	}

#ifdef ENABLE_ENVIRONNEMENT_SYSTEM
	if(m_environnementSystem)
	{
		delete m_environnementSystem;
	}
	m_environnementSystem = M2_NEW Environnement();
#endif
#ifdef ENABLE_METEO_SYSTEM
	if(m_meteoSystem)
	{
		delete m_meteoSystem;
	}
	m_meteoSystem = M2_NEW Meteo();
#endif

	if(m_itemBloquerSystem)
	{
		delete m_itemBloquerSystem;
	}
	m_itemBloquerSystem = M2_NEW ItemBloquer();
	...
	if (IsPC())
	{
		if(m_offlineShopSystem)
		{
			delete m_offlineShopSystem;
		}
		m_offlineShopSystem = M2_NEW OfflineShop();

		if(m_magasinierSystem)
		{
			delete m_magasinierSystem;
		}
		m_magasinierSystem = M2_NEW Magasinier();

		if(m_antiCheatSystem)
		{
			delete m_antiCheatSystem;
		}
		m_antiCheatSystem = M2_NEW AntiCheat();

		if(m_MobScaleSystem)
		{
			delete m_MobScaleSystem;
		}
		m_MobScaleSystem = M2_NEW MobScale();

		if(m_nouveauMouvementSystem)
		{
			delete m_nouveauMouvementSystem;
		}
		m_nouveauMouvementSystem = M2_NEW NouveauMouvement();

		if(m_CadeauSystem)
		{
			delete m_CadeauSystem;
		}
		m_CadeauSystem = M2_NEW SystemeCadeau();

		if (m_charInventaireSystem)
		{
			delete m_charInventaireSystem;
		}
		m_charInventaireSystem = M2_NEW CharInventaire();

		if (SYSTEM_TITRE > 0)
		{
			if (m_titreSystem)
			{
				delete m_titreSystem;
			}
			m_titreSystem = M2_NEW TitreSysteme();
		}
	}
...
void CHARACTER::ComputePoints()
{
	...
	if (SYSTEM_NOUVEAU_SKILL_7ET8 > 0)
	{
		SetPoint(POINT_7EMESKILL, GetPoint(POINT_7EMESKILL));
		SetPoint(POINT_8EMESKILL, GetPoint(POINT_8EMESKILL));
	}
	...
	if (SYSTEM_TITRE > 0)
	{
		if (m_titreSystem != NULL)
		{
			SetPoint(POINT_TITLE, m_titreSystem->niveauTitreTueur);
		}
	}
	long iMaxHP, iMaxSP;
	int iMaxStamina;
...
bool CHARACTER::Move(long x, long y)
{
	if (m_titreSystem != NULL)
	{
		if(m_titreSystem->AFK == true)
		{
			m_titreSystem->AFK = false;
			UpdatePacket();
		}
	}
	// °°Àº À§Ä¡¸é ÀÌµ¿ÇÒ ÇÊ¿ä ¾øÀ½ (ÀÚµ¿ ¼º°ø)
...
void CHARACTER::PointChange(BYTE type, long long amount, bool bAmount, bool bBroadcast)
{
	...
	switch (type)
	{
		...
		case POINT_LEVEL:
			...
			sys_log(0, "LEVELUP: %s %d NEXT EXP %lld", GetName(), GetLevel(), GetNextExp());
			...
			if (GetJob() == JOB_WOLFMAN)
			{
				if ((5 <= val) && (GetSkillGroup()!=1))
				{
					...
					SetSkillLevel(170, 59);
					SetSkillLevel(171, 59);
					SetSkillLevel(172, 59);
					SetSkillLevel(173, 59);
					SetSkillLevel(174, 59);
					SetSkillLevel(175, 59);
		...
		case POINT_EXP:
			{
				...
				if (amount < 0 && (long long)exp < -amount)
				{
					sys_log(1, "%s AMOUNT < 0 %d, CUR EXP: %d", GetName(), -amount, exp);
					amount = -static_cast<long long>(exp);
		...
		case POINT_LEVEL_STEP:
			if (amount > 0)
			{
				...
				switch (val)
				{
					...
					case 2:
					case 3:
						if (GetLevel() < 120) PointChange(POINT_STAT, 1);
						break;
		...
		case POINT_HP:
			{
				if (IsDead() || IsStun() || m_invincibleStatut)
					return;
		...
		case POINT_SKILL:
			...
			break;
		case POINT_STAT:
			if (GetPoint(type) + amount > 344)
			{
				sys_err("POINT_STAT dépasse les %d%%!! type bonus: %d name: %s amount %d", 344, type, GetName(), amount);
				amount = 344 - GetPoint(type);
			}
			SetPoint(type, GetPoint(type) + amount);
			val = GetPoint(type);
			break;
		...
		case POINT_RESIST_MAGIC:
			...
			{
				sys_err("RESIST_MAGIC dépasse les %d%%!! type bonus: %d name: %s amount %d", 100, type, GetName(), amount);
		...
		case POINT_TITLE:
			...
			val = GetPoint(type);
			break;
		case POINT_7EMESKILL:
			SetPoint(type, amount);
			val = GetPoint(type);
			break;
		case POINT_8EMESKILL:
			SetPoint(type, amount);
			val = GetPoint(type);
			break;
		case POINT_CINEMATIQUE:
			SetPoint(type, amount);
			val = GetPoint(type);
			break;
		case POINT_CHEQUE:
			{
#ifdef ENABLE_CHEQUE_SYSTEM
				const int64_t nTotalWon = static_cast<int64_t>(GetCheque()) + static_cast<int64_t>(amount);
				...
				val = GetCheque();
				ChatPacket(CHAT_TYPE_COMMAND, "Won %d", val);
#endif
			}
			break;
		default:
...
void CHARACTER::ApplyPoint(BYTE bApplyType, int iVal)
{
	switch (bApplyType)
	{
		...
		case APPLY_BLEEDING_PCT:		// 92
		case APPLY_BLEEDING_REDUCE:		// 93
		case APPLY_RESIST_WOLFMAN:		// 94
		case APPLY_RESIST_CLAW:			// 95
		case APPLY_ATTBONUS_WOLFMAN:		// 96
		case APPLY_ACCE_ABSORB:			// 97
...
void CHARACTER::OnClick(LPCHARACTER pkChrCauser)
{
	...
	if (!IsPC())
	{
		if (GetHerraShop())
		{
			...
			GetHerraShop()->AddGuest(pkChrCauser, GetVID(), false);
			pkChrCauser->SetShopOwner(this);
			return;
		}
...
{ 
	...
	std::unique_ptr<SQLMsg> pmsg(DBManager::instance().DirectQuery("SELECT port FROM common_alpha.map_list WHERE channel=%d AND map_index=%d", channel,lMapIndex)); 
	...
	m_posWarp.x = x; 
	m_posWarp.y = y;
	sys_log(0, "WarpSet %s %d %d current map %d target map %d port %d", GetName(), x, y, GetMapIndex(), lMapIndex,port);
	long lAddr1; 
	long lMapIndex1=0; 
	WORD wPort1;
	if (!CMapLocation::instance().Get(x, y, lMapIndex1, lAddr1, wPort1))
	{
		sys_err("cannot find map location index %d x %d y %d name %s", lMapIndex1, x, y, GetName()); 
		return false; 
	}
	TPacketGCWarp p;
...
void CHARACTER::LoadSafebox(int iSize, DWORD dwGold, int iItemCount, TPlayerItem * pItems)
{
	...
	if (!bLoaded)
	{
		for (int i = 0; i < iItemCount; ++i, ++pItems)
		{
			...
			item->SetItemLook(pItems->dwVnum);
...
void CHARACTER::IncreaseComboHackCount(int k)
{
	...
	{
		if (GetJob() == JOB_WOLFMAN || GetJob() == JOB_SURA || GetJob() == JOB_ASSASSIN || GetJob() == JOB_SHAMAN || GetJob() == JOB_WARRIOR  && m_iComboHackCount <= 10000)
		{
			return;
		}
...
void CHARACTER::ban(char* reason2, unsigned int myid2)
{
	...
	snprintf(query, 1024, "UPDATE account_alpha.account SET status='BLOCK' WHERE id='%u'", myself);
	...
	snprintf(query2, 1024, "INSERT INTO account_alpha.ban_list (account, reason, source, date, action) VALUES('%u', '%s', '%u', NOW(), 'ban')", myself, reason2, myid2);
...
{
	char * query = (char*) malloc(sizeof(*query));
	snprintf(query, 1024, "UPDATE account_alpha.account INNER JOIN player_alpha.player ON player.account_id=account.id SET status='OK' WHERE player.name='%s'", name);
	...
	snprintf(query2, 1024, "INSERT INTO account_alpha.ban_list (account, reason, source, date, action) VALUES(0, '%s', '%u', NOW(), 'unban')", name, GetPlayerID());
	//snprintf(query, 1024, "UPDATE account.account SET status='OK' WHERE id='%u'", GetAID());
	SQLMsg * msg2 = DBManager::instance().DirectQuery(query2);
	if(!msg2) {
		sys_err("cmd_unban: MySQL Query failed!");
...
void CHARACTER::PlayerPunish(bool PowerPunish, int Duration)
{
    ...
    {
        std::unique_ptr<SQLMsg> msg(DBManager::instance().DirectQuery("UPDATE account_alpha.account SET availDt = FROM_UNIXTIME(UNIX_TIMESTAMP(CURRENT_TIMESTAMP()) + %i) WHERE id = %d", Duration, GetAID()));
...
void CHARACTER::CostumeBonusTransfer(DWORD cell1, DWORD cell2)
{
	...
	{
        	ChatPacket(CHAT_TYPE_INFO, "Vous ne pouvez pas transférer de bonus avec un magasin ouvert.");
		return;
	}
	...
	if (!costume1)
	{
        	ChatPacket(CHAT_TYPE_INFO, "Il n'y a pas de costume dans cette emplacement.");
		return;
	}
    
	if (costume1->GetType() != ITEM_COSTUME || costume1->GetType() == ITEM_COSTUME && costume1->GetSubType() != COSTUME_BODY)
	{
        	ChatPacket(CHAT_TYPE_INFO, "Cette objet n'est pas un costume !");
		return;
	}
    
	if (costume1->IsEquipped())
	{
		ChatPacket(CHAT_TYPE_INFO, "Vous ne pouvez pas transférer de bonus sur un costume équipé.");
		return;
	}
	...
	if (CountSpecifyItem(71084) < 1)
	{
		ChatPacket(CHAT_TYPE_INFO, "Vous devez posséder un objet de transfère de bonus costume.");
		return;
	}
    
	if (costume2->GetAttributeCount() < 1)
	{
		ChatPacket(CHAT_TYPE_INFO, "Votre deuxième costume ne possède aucun attribut.");
		return;
	}
    
    
	RemoveSpecifyItem(71084, 1);
	...
	ChatPacket(CHAT_TYPE_INFO, "Le transfère de bonus sur votre costume a été effectué avec succès !");
}