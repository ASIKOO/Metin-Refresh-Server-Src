...
void Acce_open(LPCHARACTER ch)
{
	...
	if (ch->IsAcceOpen())
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("La fenêtre de création des étoles est déjà ouverte."));
		return;
	}
...
bool Acce_make(LPCHARACTER ch)
{
	...
	if (!(ch)->IsAcceOpen())
	{
		(ch)->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("La fenêtre de création des étoles n'est pas ouverte."));
		return false;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Déplacez l'objet dans la fenêtre."));
		return false;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("µ·ÀÌ ºÎÁ·ÇÏ°Å³ª ¾ÆÀÌÅÛÀÌ Á¦ÀÚ¸®¿¡ ¾ø½À´Ï´Ù."));
		return false;
	}
	...
	if (percent_number <= acce_proto->percent)
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("Fusion réussi avec succès."));
	...
	else
	{
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "Echoué !");
...
bool Acce_absorption_make(LPCHARACTER ch)
{
	...
	if (!(ch)->IsAcceOpen())
	{
		(ch)->ChatPacket(CHAT_TYPE_INFO, "La fenêtre de création des étoles n'est pas ouverte.");
		return false;
	}
	...
	{
		(ch)->ChatPacket(CHAT_TYPE_INFO, "Déplacez l'objet dans la fenêtre.");
		return false;
	}
	else if (items[1]->GetType() != ITEM_WEAPON && items[1]->GetType() != ITEM_ARMOR)
	{
		(ch)->ChatPacket(CHAT_TYPE_INFO, "Vous ne pouvez uniquement absorber les bonus d'une arme et armure.");
		return false;
	}
	else if (items[0]->GetAcceAttributeCount() > 0)
	{
		(ch)->ChatPacket(CHAT_TYPE_INFO, "Vous devez tout d'abord supprimer le bonus de l'étole avant de continuer.");
		return false;
	}
	...
	ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("La fusion est réussi avec succès."));
