				...
				if (loader.GetTokenVector(buf, &pTok))
				{
					...
					if (!GetVnumByOriginalName(name.c_str(), dwVnum))
					{
						...
						else
						{
							...
							{
								sys_err("ReadSpecialDropItemFile : l'item item %s  n'existe pas dans l'item_proto: node %s", name.c_str(), stName.c_str());
								existant = false;
							}
						}
					}
			...
			if (loader.GetTokenVector(buf, &pTok))
			{
				...
				if (!GetVnumByOriginalName(name.c_str(), dwVnum))
				{
					...
					else
					{
						...
						if (!ITEM_MANAGER::instance().GetTable(dwVnum))
						{
							sys_err("ReadSpecialDropItemFile : l'item %s n'existe pas dans l'item_proto: node %s", name.c_str(), stName.c_str());
							existant = false;
						}
					}
				}
			...
			for (int k = 1; k < 256; ++k)
			{
				...
				if (loader.GetTokenVector(buf, &pTok))
				{
					...
					if (!GetVnumByOriginalName(name.c_str(), dwVnum))
					{
						...
						if (!ITEM_MANAGER::instance().GetTable(dwVnum))
						{
							sys_err("ReadMonsterDropItemGroup : l'item %s n'existe pas dans l'item_proto: node %s : vnum %d", name.c_str(), stName.c_str(), dwVnum);
							existant = false;
						}
					}
			...
			for (int k = 1; k < 256; ++k)
			{
				...
				if (loader.GetTokenVector(buf, &pTok))
				{
					...
					if (!GetVnumByOriginalName(name.c_str(), dwVnum))
					{
						...
						if (!ITEM_MANAGER::instance().GetTable(dwVnum))
						{
							sys_err("ReadDropItemGroup : l'item %s n'existe pas: node %s", name.c_str(), stName.c_str());
							existant = false;
						}
					}
