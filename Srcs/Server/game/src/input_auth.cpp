...
void CInputAuth::Login(LPDESC d, const char * c_pData)
{
	...
	std::unique_ptr<SQLMsg> msg(DBManager::instance().DirectQuery("UPDATE account_alpha.account SET mac = '%s' WHERE login = '%s'", pinfo->Hwid, login));

	char szQuery[1024+1];
	snprintf(szQuery, sizeof(szQuery), "SELECT * FROM account_alpha.account WHERE mac = '%s';", pinfo->Hwid);
	std::unique_ptr<SQLMsg> msg2(DBManager::instance().DirectQuery(szQuery));
	if (msg2->Get()->uiNumRows != 0)
	{
		char szQuery2[1024+1];
		snprintf(szQuery2, sizeof(szQuery2), "SELECT * FROM account_alpha.macban WHERE mac = '%s';", pinfo->Hwid);
		std::unique_ptr<SQLMsg> msg3(DBManager::instance().DirectQuery(szQuery2));
