	...
	bool CQuestManager::Click(unsigned int pc, LPCHARACTER pkChrTarget)
	{
		...
		if (pPC)
		{
			...
			if (pkChrTarget->IsNPC())
			{
				...
				if (it == m_mapNPC.end())
				{
					if (std::find(liste_vnum_shop_vector.begin(), liste_vnum_shop_vector.end(),pkChrTarget->GetRaceNum())!=liste_vnum_shop_vector.end() == false) // Patch syserr magasin.
					{
						if ((pkChrTarget->GetRaceNum() >= 34001 && pkChrTarget->GetRaceNum() <= 34069) == false) // Patch syserr Familier.
						{
