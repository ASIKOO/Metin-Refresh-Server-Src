...
static void next (LexState * ls)
{
	...
	if (ls->lookahead.token != TK_EOS)
	{
		...
		ls->t.token = luaX_llex2(ls, &ls->t.seminfo);  /* read next token */
}
...
static void lookahead (LexState * ls)
{
	...
	ls->lookahead.token = luaX_llex2(ls, &ls->lookahead.seminfo);
}
...
void parse(char * filename)
{
	...
	ZIO z;
	luaZ_init(L, &z, getF,&lf,"quest");
	Mbuffer b;
	struct LexState lexstate;
	pls = &lexstate;
	luaZ_initbuffer(L,&b);
	lexstate.buff = &b;
	luaX_setinput2(L, &lexstate, &z, luaS_new(L, zname(&z)));
	...
	while (1)
	{
		printf("Initializing new procedure now.");
		next(&lexstate);
		printf("next 1");
		/*/
		...
		Token& t= lexstate.t;
		printf("flag: %d with case %d ", t.token, ps);
		switch(ps)
		{
			case ST_START:
				{
			...
			case ST_QUEST:
				{
					...
					if (t.token == TK_NAME || t.token == TK_STRING)
					{
						...
						error("quest name must be given. Current token: ");
				}
				break;
			case ST_QUEST_WITH_OR_BEGIN:
				...
				if (t.token == TK_WITH)
				{
					...
					//cout << (lexstate.t);
					next(&lexstate);
					while (lexstate.t.token!=TK_BEGIN)
					{
						os << " " <<(lexstate.t);
				...
				else
				{
					...
					printf("Error with current flag: %d", t.token);
					error(os.str().c_str());
				}
				break;
			case ST_STATELIST:
			...
			case ST_WHEN_WITH_OR_BEGIN:
				{
					printf("Start to check with condition");
					assert(nested==2);
					printf("check. Proceed 1");
					current_when_condition = "";
					if (t.token == TK_WITH)
					{
						printf("check. Proceed 2");
						// here comes 조건식
						next(&lexstate);
						ostringstream os;
						os << (lexstate.t);
			...
			case ST_WHEN_BODY:
				{
					printf("lookup for end begins now!");
					assert(nested==3);
					...
					while (1)
					{
						printf("Now check when body. Check 1");
						...
						printf("Now check when body. Check 2");
			...
			case ST_FUNCTION_ARG:
				{
					...
					if (t.token != ')')
					{
						do 
						{
							...
							{
								ostringstream os;
								os << "invalid argument name " << getstr(t.seminfo.ts) << " for function " << current_function_name;
								error(os.str().c_str());
							}
