	...
	case INVENTORY:
		{
			...
			else if (bCell >= inventorysize || bCell >= m_charInventaireSystem->GetMaxPlaceInventaire())
				return false;
			...
			if (m_pointsInstant.bItemGrid[bCell])
			{
				if (m_pointsInstant.bItemGrid[bCell] == iExceptionCell)
				{
					...
					int j = 1;
					BYTE bPage;
					bPage = bCell / (INVENTORY_MAX_NUM /4);
					do
					{
						BYTE p = bCell + (5 * j);

						if (p >= inventorysize || p >= m_charInventaireSystem->GetMaxPlaceInventaire())
							return false;
			...
			else
			{
				...
				do
				{
					...
					if (bigger_inventory == true && (p / (INVENTORY_MAX_NUM /4) != bPage))
						return false;
					if (bigger_inventory == false && (p / (180 / 4) != bPage))
						return false;
...
void CHARACTER::__OpenSelectedPrivateShop()
{
	/*unsigned bodyPart = GetPart(PART_MAIN);
	switch (bodyPart)
	{
	...
	{
		ChatPacket(CHAT_TYPE_COMMAND, "OpenSelectPrivateShop");
	}
	else
	{
		ChatPacket(CHAT_TYPE_INFO, "Le système de personnalisation des magasins est désactivé.");
	}
}
...
{
	if(!UtilisationObjet(this, item, true))
	{
		return false;
	}
...
//				iLimitTimerBasedOnWearFlagIndex = i;
	...
	{
		ChatPacket(CHAT_TYPE_COMMAND, "OpenEnchantItemPlus");
	}
#endif
#ifdef NEW_PET_SYSTEM
		...
		if (pmsg2->Get()->uiNumRows > 0) {
			...
			if (atoi(row[0]) > 0) {
				if (GetNewPetSystem()->IsActivePet()) {
					ChatPacket(CHAT_TYPE_INFO, "Vous devez invoquer votre animal de compagnie.");
					return false;
				}

				DBManager::instance().DirectQuery("UPDATE new_petsystem SET duration =(tduration) WHERE id = %d", item2->GetID());
				ChatPacket(CHAT_TYPE_INFO, "La vie de votre animal est à présent complète.");
			}
			else {
				DBManager::instance().DirectQuery("UPDATE new_petsystem SET duration =(tduration/2) WHERE id = %d", item2->GetID());
				ChatPacket(CHAT_TYPE_INFO, "La vie de votre animal est restauré.");
			}
			item->SetCount(item->GetCount() - 1);
			return true;
		}
		...
		if (item2 = GetItem(DestCell)) {
			if (item2->GetVnum() == 55002) {
				if(item2->GetAttributeValue(0) > 0){
					ChatPacket(CHAT_TYPE_INFO, "La boîte a déjà un animal de compagnie à l'intérieur.");
				}
				else{
					if (!IsValidItemPosition(DestCell) || !(item2 = GetItem(DestCell)))
						return false;
							...
							else
							{
								switch (item->GetVnum())
								{
									...
									case 72003:
									{
										...
										if (item->GetValue(0) <= 180)
										{
											now += item->GetValue(0) * 60;
										}
										else
										{
											now += item->GetValue(0);
										...
										const char* dateExpire = std::string(szCreateDate).c_str();
										std::unique_ptr<SQLMsg> msg(DBManager::instance().DirectQuery("UPDATE account_alpha.account SET silver_expire='%s' WHERE id='%d';", dateExpire, GetAID()));
										ChatPacket(CHAT_TYPE_INFO, "Votre anneau d'expérience est activé, elle expirera le %s", dateExpire);
										ChatPacket(CHAT_TYPE_INFO, "Veuillez vous reconnectez pour terminer son activation.");
										ChatPacket(CHAT_TYPE_COMMAND, "PhaseSelectPlayer");*/
										item->SetCount(item->GetCount() - 1);
									...
									case 72006:
									{
										...
										tm1 = localtime(&now);
    										strftime(szCreateDate, 255, "%Y-%m-%d %H:%M:%S", tm1);
										const char* dateExpire = std::string(szCreateDate).c_str();
										std::unique_ptr<SQLMsg> msg(DBManager::instance().DirectQuery("UPDATE account_alpha.account SET gold_expire='%s' WHERE id='%d';", dateExpire, GetAID()));
										ChatPacket(CHAT_TYPE_INFO, "Votre gant du voleur est activé, elle expirera le %s", dateExpire);
										ChatPacket(CHAT_TYPE_INFO, "Veuillez vous reconnectez pour terminer son activation.");
										ChatPacket(CHAT_TYPE_COMMAND, "PhaseSelectPlayer");*/
										item->SetCount(item->GetCount() - 1);
									...
									case 72018:
									{
										...
										tm1 = localtime(&now);
    										strftime(szCreateDate, 255, "%Y-%m-%d %H:%M:%S", tm1);
										const char* dateExpire = std::string(szCreateDate).c_str();
										std::unique_ptr<SQLMsg> msg(DBManager::instance().DirectQuery("UPDATE account_alpha.account SET autoloot_expire='%s' WHERE id='%d';", dateExpire, GetAID()));
										ChatPacket(CHAT_TYPE_INFO, "Votre troisième main est activé, elle expirera le %s", dateExpire);
										ChatPacket(CHAT_TYPE_INFO, "Veuillez vous reconnectez pour terminer son activation.");
										ChatPacket(CHAT_TYPE_COMMAND, "PhaseSelectPlayer");*/
										item->SetCount(item->GetCount() - 1);
									...
									case 72024:
									{
										...
										tm1 = localtime(&now);
    										strftime(szCreateDate, 255, "%Y-%m-%d %H:%M:%S", tm1);
										const char* dateExpire = std::string(szCreateDate).c_str();
										std::unique_ptr<SQLMsg> msg(DBManager::instance().DirectQuery("UPDATE account_alpha.account SET money_drop_rate_expire='%s' WHERE id='%d';", dateExpire, GetAID()));
										ChatPacket(CHAT_TYPE_INFO, "Votre médaille de chance est activé, elle expirera le %s", dateExpire);
										ChatPacket(CHAT_TYPE_INFO, "Veuillez vous reconnectez pour terminer son activation.");
										ChatPacket(CHAT_TYPE_COMMAND, "PhaseSelectPlayer");*/
										item->SetCount(item->GetCount() - 1);
				...
				switch (item->GetSubType())
				{
...
#ifdef ENABLE_NEW_PRIVATE_SHOP_SYSTEM
							case 71221: // º¸µû¸®
								if (LC_IsYMIR() == true || LC_IsKorea() == true)
								{
									if (IS_BOTARYABLE_ZONE(GetMapIndex()) == true)
									{
										__OpenSelectedPrivateShop();
									}
									else
									{
										ChatPacket(CHAT_TYPE_INFO, LC_TEXT("°³ÀÎ »óÁ¡À» ¿­ ¼ö ¾ø´Â Áö¿ªÀÔ´Ï´Ù"));
					...
					case USE_DETACHMENT:
						{
							...
							if (item2->IsEquipped())
							{
								ChatPacket(CHAT_TYPE_INFO, "Impossible de retirer les pierres d'un objet équipé.");
								return false;
							}
					...
					case USE_ADD_ATTRIBUTE2:
						...
						case USE_RESET_SASH_ATTR:
						{
							...
							if (item2->GetType() == ITEM_COSTUME && item2->GetSubType() == COSTUME_ACCE)
							{
								if (item2->GetAcceAttributeCount() <= 0)
									return false;
								...
								item->RemoveFromCharacter();
								ChatPacket(CHAT_TYPE_INFO, "Votre étole a été réinitialisé.");		
							}
							else
							{
								return false;
		...
		case ITEM_METIN:
			{
				...
				for (i = 0; i < ITEM_SOCKET_MAX_NUM; ++i)
					...
					{
						if (pierre_instant_ajout == 0)
						{
						// ¼® È®·ü
							if (number(1, 100) <= pierre_pourcent_ajout)
							{
...
bool CHARACTER::DropItem(TItemPos Cell, BYTE bCount)
{
	...
	if (!m_charInventaireSystem->GetInventaire(this, Cell.cell, item->GetSize()) && INVENTORY == Cell.window_type)
		return false;
...
bool CHARACTER::DestroyItem(TItemPos Cell)
{
    ...
    if (item->GetVnum() == 71084 || item->GetID() == 71085 || item->GetVnum() == 39028 || item->GetVnum() == 39029 || item->GetVnum() == 71051 || item->GetVnum() == 71052)
    {
    	ChatPacket(CHAT_TYPE_INFO, "Vous ne pouvez pas détruire ces objets.");
	return false;
    }
    ITEM_MANAGER::instance().RemoveItem(item);
    ChatPacket(CHAT_TYPE_INFO, "Votre %s est détruit.", item->GetName());
    return true;
}
	...
	if (item)
	{
		...
		if (item->AddToGround(GetMapIndex(), pos))
		{
			...
			if(gold_drop_time>=60)
				ChatPacket(CHAT_TYPE_INFO, LC_TEXT("¶³¾îÁø ¾ÆÀÌÅÛÀº %dºÐ ÈÄ »ç¶óÁý´Ï´Ù."), gold_drop_time/60);
			else
...
bool CHARACTER::MoveItem(TItemPos Cell, TItemPos DestCell, BYTE count)
{
	...
	if (!m_charInventaireSystem->GetInventaire(this, DestCell.cell, item->GetSize()) && INVENTORY == DestCell.window_type)
		return false;
...
bool CHARACTER::EquipItem(LPITEM item, int iCandidateCell)
{
	...
	if (true == item->IsEquipped())
	{
		...
		else if (dwVnum >= 85011 && dwVnum <= 85018)
		{
			this->EffectPacket(SE_EQUIP_ACCE_4);
		}
...
//	BYTE itemSubType = item->GetSubType();
	if (item->GetType() == ITEM_COSTUME && item->GetSubType() >= COSTUME_WEAPON_SWORD)
	{
		...
		if (!weapon)
		{
			ChatPacket(CHAT_TYPE_INFO, "Vous devez tout d'abord vous équipez d'une arme.");
			return false;
		}
		if (weapon->GetSubType() == 8 && weapon->GetSubType() + 7 == item->GetSubType()){

		}
		else if (weapon->GetSubType() + COSTUME_WEAPON_SWORD != item->GetSubType())
		{
			ChatPacket(CHAT_TYPE_INFO, "Vous devez vous équipez avec une arme du même type que celle du costume.");
			return false;
		}
	}
	if (item->GetType() == ITEM_WEAPON)
...
#endif
				&& weapon->GetSubType() == COSTUME_WEAPON_BOW)
			{
					// Accepte le changement de flèche
			}
			else if (weapon->GetSubType() - COSTUME_WEAPON_SWORD != item->GetSubType())
			{
				ChatPacket(CHAT_TYPE_INFO, "Vous devez vous équipez avec une arme du même type que celle du costume.");
...
{	
	...
	if (IS_SET(item->GetFlag(), ITEM_FLAG_IRREMOVABLE))
		return false;
#ifdef ENABLE_QUIVER_SYSTEM
	if (GetWear(WEAR_COSTUME_WEAPON) && item->GetType() == ITEM_WEAPON && item->GetSubType() != WEAPON_ARROW && item->GetSubType() != WEAPON_QUIVER)
	{
#else
	if (GetWear(WEAR_COSTUME_WEAPON) && item->GetType() == ITEM_WEAPON && item->GetSubType() != WEAPON_ARROW)
	{
#endif
		ChatPacket(CHAT_TYPE_INFO, "Vous devez vous déséquipez de l'arme costume.");
		return false;
	}
...
bool CHARACTER::SameItem(LPITEM item1, LPITEM item2)
{
	...
	if (item1->GetSubType() != item2->GetSubType())
		return false;
	...
	int antiflagItem1 = 0;
	int antiflagItem2 = 0;
	switch(table1->dwAntiFlags)
	{
		case ITEM_ANTIFLAG_WARRIOR:
			antiflagItem1 = 1;
		break;
		case ITEM_ANTIFLAG_ASSASSIN:
			antiflagItem1 = 2;
		break;
		case ITEM_ANTIFLAG_SURA:
			antiflagItem1 = 3;
		break;
		case ITEM_ANTIFLAG_SHAMAN:
			antiflagItem1 = 4;
		break;
		case ITEM_ANTIFLAG_WOLFMAN:
			antiflagItem1 = 5;
		break;
	}
	switch(table2->dwAntiFlags)
	{
		case ITEM_ANTIFLAG_WARRIOR:
			antiflagItem2 = 1;
		break;
	...
	if (item1->GetType() == ITEM_WEAPON || item2->GetType() == ITEM_WEAPON)
	{
		if (item1->GetSubType() == WEAPON_ARROW || item2->GetSubType() == WEAPON_ARROW || item1->GetSubType() == WEAPON_QUIVER || item2->GetSubType() == WEAPON_QUIVER)
		{
			return false;
		}
	}

	if (antiflagItem1 != antiflagItem2)
	{
		return false;
	}
	else
