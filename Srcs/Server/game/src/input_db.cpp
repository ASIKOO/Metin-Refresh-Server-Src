...
void CInputDB::PlayerLoad(LPDESC d, const char * data)
{
	...
	if (!SECTREE_MANAGER::instance().GetValidLocation(pTab->lMapIndex, pTab->x, pTab->y, lMapIndex, pos, d->GetEmpire()))
	{
		...
		snprintf(szQuery, sizeof(szQuery), "UPDATE player%s SET map_index=%ld , x=%ul , y=%ul WHERE `name`='%s'", get_table_postfix(), g_start_map[d->GetEmpire()], g_start_position[d->GetEmpire()][0], g_start_position[d->GetEmpire()][1], pTab->name);
...
void CInputDB::ItemLoad(LPDESC d, const char * c_pData)
{
	...
	for (DWORD i = 0; i < dwCount; ++i, ++p)
	{
		...
		LPITEM item = ITEM_MANAGER::instance().CreateItem(p->vnum, p->count, p->id, p->dwVnum);
#else
