	...
	if (GetMobRank() >= MOB_RANK_BOSS && !IsStone() && GetMobTable().dwGoldMax != 0)
	{
		...
		for (int i = 0; i < iSplitCount; ++i)
		{
			...
			if (test_server)
			{
				...
				sys_log(0, "Drop Money gold %lld GoldMin %d GoldMax %d", iGold, GetMobTable().dwGoldMax, GetMobTable().dwGoldMax);
			}
...
void CHARACTER::Dead(LPCHARACTER pkKiller, bool bImmediateDead)
{
	...
	if (pkKiller && pkKiller->IsPC())
	{
		...
		if (pkKiller->IsPC() == true)
		{
			if (IsMonster() == false)
			{
				if (m_antiCheatSystem != NULL)
				{
					m_antiCheatSystem->AntiCheatPvP(pkKiller, this);
				}
			}
		}
...
bool CHARACTER::Damage(LPCHARACTER pAttacker, int dam, EDamageType type) // returns true if dead
{
	...
	if (IsAffectFlag(AFF_MANASHIELD))
	{
		...
		if(!bugfix_sura_manashield)
		{
			...
			int iDamageSPPart = dam / degat_protection_sura;
			...
			int iDamageToSP = iDamageSPPart * 0.4f;
			int iSP = GetSP();
			...
			{
				PointChange(POINT_SP, -iDamageToSP);
				float damTemp = (iDamageSPPart/degat_protection_sura) * 4;
				dam -= (iDamageSPPart/degat_protection_sura) - (int)damTemp;
			}
			else
			{
				// 정신력이 모자라서 피가 더 깍여야할떄
				PointChange(POINT_SP, -GetSP());
				float damTemp = ((iSP/20 * 100)) * 4;
				dam -= ((iSP / 20) * 100) - (int)damTemp;
			}
		}
	}
...
int CHARACTER::GetArrowAndBow(LPITEM * ppkBow, LPITEM * ppkArrow, int iArrowCount/* = 1 */)
{
	...
	if (!(pkArrow = GetWear(WEAR_ARROW)) || pkArrow->GetType() != ITEM_WEAPON ||
			...
			(pkArrow->GetProto()->bSubType != WEAPON_ARROW && pkArrow->GetProto()->bSubType != WEAPON_QUIVER))
#else
	...
	else if (pkArrow->GetProto()->bSubType == WEAPON_ARROW)
	{
...
#ifdef ENABLE_QUIVER_SYSTEM
void CHARACTER::UseArrow(LPITEM pkArrow, DWORD dwArrowCount, LPCHARACTER ch, LPCHARACTER victim)
#else
void CHARACTER::UseArrow(LPITEM pkArrow, DWORD dwArrowCount)
#endif
{
#ifdef ENABLE_QUIVER_SYSTEM
	bool QuiverEquip = false;
	if (pkArrow->GetSubType() == WEAPON_QUIVER)
	{
		QuiverEquip = true;
	}
	if (QuiverEquip == false)
	{
	...
	else
	{
		...
		{
			for (int i = 0; i < dwArrowCount; i++)
			{
				ch->CreateFly(FLY_QUIVER_ATTACK_NORMAL, victim);
			}
		}
	}
#else
...
#ifdef ENABLE_QUIVER_SYSTEM
							m_me->UseArrow(pkArrow, 1, m_me, pkVictim);
#else
...
#ifdef ENABLE_QUIVER_SYSTEM
								m_me->UseArrow(pkArrow, 2 + (m_me->GetSkillPower(SKILL_YEONSA) *6/100), m_me, pkVictim);
#else
				...
				case SKILL_KWANKYEOK:
					{
						...
						if (iUseArrow == m_me->GetArrowAndBow(&pkBow, &pkArrow, iUseArrow))
						{
							...
							m_me->ComputeSkill(m_bType, pkVictim);
#ifdef ENABLE_QUIVER_SYSTEM
								m_me->UseArrow(pkArrow, iUseArrow, m_me, pkVictim);
#else
								m_me->UseArrow(pkArrow, iUseArrow);
#endif
						}
