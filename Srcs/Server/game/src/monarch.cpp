...
int CMonarch::HealMyEmpire(LPCHARACTER ch ,long long price)
{
	BYTE Empire = ch->GetEmpire();
	DWORD pid = ch->GetPlayerID();

	sys_log(0, "HealMyEmpire[%d] pid:%d price %lld", pid, Empire, price);
...
bool CMonarch::IsMoneyOk(long long price, BYTE bEmpire)
{
	dev_log(LOG_DEB1, "GetMoney(%lld), price = (%lld,%lld)", bEmpire, GetMoney(bEmpire), price);
	return (GetMoney(bEmpire) >= price);
}

bool CMonarch::SendtoDBAddMoney(long long Money, BYTE bEmpire, LPCHARACTER ch)
{
	if (GetMoney(bEmpire) + Money > 9223372036854775807LLU)
		return false;
	...
	int nEmpire = bEmpire;	
	db_clientdesc->DBPacketHeader(HEADER_GD_ADD_MONARCH_MONEY, ch->GetDesc()->GetHandle(), sizeof(int) + sizeof(long long));
	db_clientdesc->Packet(&nEmpire, sizeof(int));	
	db_clientdesc->Packet(&Money, sizeof(long long));	
	return true;
}

bool CMonarch::SendtoDBDecMoney(long long Money, BYTE bEmpire, LPCHARACTER ch)
{
	...
	db_clientdesc->DBPacketHeader(HEADER_GD_DEC_MONARCH_MONEY, ch->GetDesc()->GetHandle(), sizeof(int) + sizeof(long long));
	db_clientdesc->Packet(&nEmpire, sizeof(int));	
	db_clientdesc->Packet(&Money, sizeof(long long));	
	return true;
}

bool CMonarch::AddMoney(long long Money, BYTE bEmpire)
{
	if (bEmpire >= _countof(m_MonarchInfo.money))
		return false;
...
bool CMonarch::DecMoney(long long Money, BYTE bEmpire)
{
	if (bEmpire >= _countof(m_MonarchInfo.money))
		return false;
...
long long CMonarch::GetMoney(BYTE bEmpire)
{
	if (bEmpire >= _countof(m_MonarchInfo.money))
		return 0;
