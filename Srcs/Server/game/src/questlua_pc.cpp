...
namespace quest 
{
	...
	int pc_change_won(lua_State* L)
	{
		int won = (int)lua_tonumber(L, -1);

		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();

		ch->PointChange(POINT_CHEQUE, won, true);
	}
#endif
	...
	int pc_polymorph(lua_State* L)
	{
		...
		if (dwVnum <= MAIN_RACE_MAX_NUM) // Si c'est un changement de race.
		{
			ch->SetRace(dwVnum);
			ch->ChatPacket(CHAT_TYPE_INFO, "Race Changer= %d", ch->GetRaceNum());
			ch->ClearSkill();
			ch->SetSkillGroup(0);
			ch->ClearSubSkill();
		}
		else // Si c'est un poly en mob
		{
...
teleport_area:
	...
	int pc_change_name(lua_State* L)
	{
		...
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		const char * ancienNom = ch->GetName();
		...
		//SQLMsg * msg2 = DBManager::instance().DirectQuery("UPDATE player_metier%s SET pseudoJoueur='%s' WHERE pseudoJoueur='%s'", get_table_postfix(), szName, ancienNom);
		SQLMsg * msg3 = DBManager::instance().DirectQuery("UPDATE player_titre%s SET pseudo='%s' WHERE pseudo='%s'", get_table_postfix(), szName, ancienNom);
		SQLMsg * msg4 = DBManager::instance().DirectQuery("UPDATE private_herra_shops%s SET owner_name='%s' WHERE owner_name='%s'", get_table_postfix(), szName, ancienNom);
		//SQLMsg * msg5 = DBManager::instance().DirectQuery("UPDATE anti_cheat_detection%s SET pseudo='%s' WHERE pseudo='%s'", get_table_postfix(), szName, ancienNom);
	...
	int pc_set_7eme_skill(lua_State * L)
	{
		...
		{
			int idSkill = lua_tonumber(L, 1);
			LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
			std::unique_ptr<SQLMsg> msg(DBManager::instance().DirectQuery("UPDATE player%s SET 7emeSkill='%d' WHERE name='%s';", get_table_postfix(), idSkill));
			ch->SetNewSkill(idSkill); 
			ch->PointChange(POINT_7EMESKILL, idSkill, false, true);
			ch->UpdatePacket();
	...
	int pc_get_7eme_skill(lua_State * L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		std::unique_ptr<SQLMsg> msg(DBManager::instance().DirectQuery("SELECT 7emeSkill FROM player%s WHERE name='%s';", get_table_postfix(), ch->GetName()));
		...
		ch->UpdatePacket();
		ch->SetNewSkill(idSkill); 
		ch->ChatPacket(CHAT_TYPE_COMMAND, "7emeSkill %d", idSkill);
		lua_pushnumber(L, idSkill);
		return 1;
	}

	int pc_get_8eme_skill(lua_State * L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		std::unique_ptr<SQLMsg> msg(DBManager::instance().DirectQuery("SELECT 8emeSkill FROM player%s WHERE name='%s';", get_table_postfix(), ch->GetName()));
		MYSQL_ROW row = mysql_fetch_row(msg->Get()->pSQLResult);
		
		int idSkill = 0;
		str_to_number(idSkill, row[0]);
		ch->PointChange(POINT_8EMESKILL, idSkill, false, true);
		ch->UpdatePacket();
		ch->SetNewerSkill(idSkill);
		ch->ChatPacket(CHAT_TYPE_COMMAND, "8emeSkill %d", idSkill);
		lua_pushnumber(L, idSkill);
		return 1;
	}


	int pc_set_8eme_skill(lua_State * L)
	{
		...
		else
		{
			int idSkill = lua_tonumber(L, 1);
			LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
			std::unique_ptr<SQLMsg> msg(DBManager::instance().DirectQuery("UPDATE player%s SET 8emeSkill='%d' WHERE name='%s';", get_table_postfix(), idSkill));
			ch->SetNewerSkill(idSkill); 
			ch->PointChange(POINT_8EMESKILL, idSkill, false, true);
			ch->UpdatePacket();
	...
	int pc_get_pourcent_titre(lua_State * L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		std::string titre = lua_tostring(L, 1);
		lua_pushnumber(L, ch->m_titreSystem->GetPourcentTitre(titre));
		return 1;
	}

	int pc_get_max_pourcent_titre(lua_State * L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		std::string titre = lua_tostring(L, 1);
		lua_pushnumber(L, ch->m_titreSystem->GetMaxPourcentTitre(titre));
		return 1;
	}


	int pc_add_pourcent_titre(lua_State * L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		std::string titre = lua_tostring(L, 1);
		int pourcent = lua_tonumber(L, 2);
		ch->m_titreSystem->AddPourcentTitre(titre, pourcent, ch);
		return 0;
	}

	int pc_get_titre_level(lua_State * L)
	{
		lua_pushnumber(L, CQuestManager::instance().GetCurrentCharacterPtr()->GetPoint(POINT_TITLE));
		return 1;
	}

	int pc_set_titre_level(lua_State * L)
	{
		...
		else
		{
			...
			LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
			ch->SetTitle(level); 
			ch->PointChange(POINT_TITLE, level);
			ch->UpdatePacket();
	...
	int pc_add_titre(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		const char* titre = lua_tostring(L, 1);
		if (SYSTEM_TITRE > 0)
		{
			if (ch->m_titreSystem != NULL)
			{
				ch->m_titreSystem->SaveTitre(ch->GetName(), titre);
			}
		}
		else
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Le système de titre n'est pas activé.");
		}
		return 0;
	}

	int pc_verif_titre(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		const char* titre = lua_tostring(L, 1);
		if (SYSTEM_TITRE > 0)
		{
			if (ch->m_titreSystem != NULL)
			{
				if(ch->m_titreSystem->VerifTitre(titre, ch->GetName()))
				{
					lua_pushboolean(L, true);
				}
				else
	...
	int pc_add_inventaire_extension(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		if (ch->m_charInventaireSystem != NULL)
		{
			ch->m_charInventaireSystem->UnlockInventaire(ch);
		}
		return 1;
	}
	
	int pc_get_inventaire_extension(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
		lua_pushnumber(L, ch->m_charInventaireSystem->inventaire_joueur);
		return 1;
	}

	int pc_get_won(lua_State* L)
	{
		LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
	...
	int pc_add_is(lua_State * L)
	{
		...
		{
			int monnaieIS = lua_tonumber(L, 1);
			LPCHARACTER ch = CQuestManager::instance().GetCurrentCharacterPtr();
			std::unique_ptr<SQLMsg> msg(DBManager::instance().DirectQuery("UPDATE account_alpha.account SET cash=cash+'%d' WHERE id='%d';", get_table_postfix(), ch->GetAID()));
	...
	void RegisterPCFunctionTable()
	{
		...
		{
			{ "add_is",				pc_add_is				},
			{ "get_inventaire_extension",		pc_get_inventaire_extension		},
			{ "add_inventaire_extension",		pc_add_inventaire_extension		},
			{ "get_titre_level",		pc_get_titre_level		},
			{ "set_titre_level",		pc_set_titre_level		},
			{ "add_titre",			pc_add_titre			},
			{ "verif_titre",		pc_verif_titre			},
			{ "get_pourcent_titre",		pc_get_pourcent_titre		},
			{ "get_max_pourcent_titre",	pc_get_max_pourcent_titre	},
			{ "add_pourcent_titre",		pc_add_pourcent_titre		},
			{ "get_7eme_skill",		pc_get_7eme_skill		},
			{ "set_7eme_skill",		pc_set_7eme_skill		},
			{ "get_8eme_skill",		pc_get_8eme_skill		},
			{ "set_8eme_skill",		pc_set_8eme_skill		},
