...
bool NewPlayerTable2(TPlayerTable * table, const char * name, BYTE race, BYTE shape, BYTE bEmpire)
{
	...
	if (race == 8)
	{
		table->x		= CREATE_START_X_WOLFMAN(bEmpire) + number(-300, 300);
		table->y		= CREATE_START_Y_WOLFMAN(bEmpire) + number(-300, 300);
	}
	else
	{
		table->x		= CREATE_START_X(bEmpire) + number(-300, 300);
...
void CInputLogin::CharacterDelete(LPDESC d, const char * data)
{
	...
	char szQuery[1024+1];
	snprintf(szQuery, sizeof(szQuery), "SELECT name FROM player%s WHERE id = '%d';", get_table_postfix(), player_delete_packet.player_id);
	std::unique_ptr<SQLMsg> msg(DBManager::instance().DirectQuery(szQuery));
	if (msg->Get()->uiNumRows > 0)
	{
		MYSQL_ROW row = mysql_fetch_row(msg->Get()->pSQLResult);
		const char* nomJoueur = row[0];
		if (SYSTEM_TITRE > 0)
		{
			std::unique_ptr<SQLMsg> pmsgDeleteTitre (DBManager::instance().DirectQuery("DELETE FROM player_titre%s WHERE pseudo='%s'", get_table_postfix(), nomJoueur));
		}
		if (SYSTEM_OFFLINE_SHOP > 0)
		{
			std::unique_ptr<SQLMsg> pmsgDeleteShop (DBManager::instance().DirectQuery("DELETE FROM private_herra_shops%s WHERE owner_id='%d'", get_table_postfix(), player_delete_packet.player_id));
			std::unique_ptr<SQLMsg> pmsgDeleteShopItem (DBManager::instance().DirectQuery("DELETE FROM private_herra_items%s WHERE owner_id='%d'", get_table_postfix(), player_delete_packet.player_id));  
		}
	}
...
void CInputLogin::Entergame(LPDESC d, const char * data)
{
	...
	if (ch->m_itemBloquerSystem != NULL)
	{
		ch->m_itemBloquerSystem->StartCheckItem(ch);
	}
	...
	SQLMsg * pMsg3 = DBManager::instance().DirectQuery("select shop_id from private_herra_shops%s WHERE owner_name = '%s' AND status='%d'", get_table_postfix(), ch->GetName(), 1);
	if (pMsg3->Get()->uiNumRows > 0)
	{
		MYSQL_ROW row = mysql_fetch_row(pMsg3->Get()->pSQLResult);
		int tempShopID = 0;
		str_to_number(tempShopID, row[0]);
		ch->HerraShopVID = tempShopID;
	}

	if (SYSTEM_CARTE_CADEAU > 0)
	{
		ch->m_CadeauSystem->ExecutionCadeau(ch);
	}
	...
	if (SYSTEM_TITRE > 0)
	{
		...
		{
			ch->m_titreSystem->ChargementTitre(ch);
			int idTitre = 0;
			SQLMsg * pMsg3 = DBManager::instance().DirectQuery("select titre from player%s WHERE name = '%s' AND status='%d'", get_table_postfix(), ch->GetName(), 1);
			if (pMsg3->Get()->uiNumRows > 0)
			{
				MYSQL_ROW row = mysql_fetch_row(pMsg3->Get()->pSQLResult);
				str_to_number(idTitre, row[0]);
				ch->m_titreSystem->niveauTitreTueur = idTitre;
			}
			if (ch->m_titreSystem->niveauTitreTueur <= 0)
			{
				ch->m_titreSystem->UpdatePacketTitre(0, ch);
			}
			else
			{
				ch->m_titreSystem->UpdatePacketTitre(ch->m_titreSystem->niveauTitreTueur, ch);
			}
			ch->UpdatePacket();
		}
	}
#ifdef ENABLE_ENVIRONNEMENT_SYSTEM
	if(ch->m_environnementSystem != NULL)
	{
		ch->m_environnementSystem->InitialisationEnvironnement(ch);
	}
#endif
#ifdef ENABLE_METEO_SYSTEM
	...
	std::unique_ptr<SQLMsg> pmsgUpdateCompte (DBManager::instance().DirectQuery("UPDATE account_alpha.account SET connect=1 WHERE id='%d'", ch->GetAID()));

	if(ch->m_nouveauMouvementSystem != NULL)
	{
		ch->m_nouveauMouvementSystem->StartUpdateNouveauMouvement(ch);
	}
	...
	if (SYSTEM_RATE > 0)
	{
		if (rateActive == 0)
		{
			CPrivManager::instance().RequestGiveEmpirePriv(0, PRIV_ITEM_DROP, RATE_DROP_ITEM, (24*365*60*60));
			CPrivManager::instance().RequestGiveEmpirePriv(0, PRIV_GOLD_DROP, RATE_DROP_GOLD, (24*365*60*60));
			CPrivManager::instance().RequestGiveEmpirePriv(0, PRIV_GOLD10_DROP, RATE_DROP_GOLD, (24*365*60*60));
			CPrivManager::instance().RequestGiveEmpirePriv(0, PRIV_EXP_PCT, RATE_DROP_EXP, (24*365*60*60));
			//CreationBot(ch);
			rateActive = 1;
		}
	}
#ifdef ENABLE_BLOCK_INVENTAIRE
	ch->m_charInventaireSystem->ChargementInventaire(ch);
#endif

#ifdef ENABLE_MOB_SCALE_SYSTEM
	SendMobScalePaquet(ch);
#endif
