...
int CItem::FindEquipCell(LPCHARACTER ch, int iCandidateCell)
{
	...
	else if (GetType() == ITEM_WEAPON && GetSubType() == WEAPON_QUIVER)
		return WEAR_ARROW;
#endif
	...
	if (false == IsAccessoryForSocket())
	{
		if (m_pProto->bType == ITEM_WEAPON || m_pProto->bType == ITEM_ARMOR)
		{
			...
			for (int i = 0; i < ITEM_SOCKET_MAX_NUM; ++i)
			{
				...
				{
#ifdef ENABLE_QUIVER_SYSTEM
					if (m_pProto->bType != ITEM_WEAPON && m_pProto->bSubType != WEAPON_QUIVER)
		...
		case ITEM_COSTUME:
			{
				...
				{
					m_pOwner->SetPart((BYTE)toSetPart, static_cast<DWORD>(toSetValue));
