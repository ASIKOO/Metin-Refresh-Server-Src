		...
		if (DESACTIVE_DROP_MANUEL <= 0)
		{
			...
			if (LC_IsKorea() || LC_IsYMIR())
			{
				if (vnum == 50300 && bTryMagic)
				{
					...
					{
						srand(time(0));
						int randomTypeManuel = rand() % 100 + 1;
						srand(time(0));
						int randomTypeManuel2 = rand() % 4 + 1;
				
						if (((randomTypeManuel >= 61 && randomTypeManuel <= 80)  || randomTypeManuel == 3)) // manuel de parade de skill
						{
							dwSkillVnum = number(221, 229);
						}
						if (((randomTypeManuel >= 81 && randomTypeManuel <= 100)  || randomTypeManuel == 4)) // manuel de bonus de skill
						{
			...
			{
				DWORD dwSkillVnum;
				int randomNombreManuel = rand() % 3 + 1;
				do
				{
					srand(time(0));
					int randomTypeManuel = rand() % 100 + 1;
					srand(time(0));
					int randomTypeManuel2 = rand() % 4 + 1;
					if ((randomTypeManuel >= 1 && randomTypeManuel <= 40) || randomTypeManuel == 1) // manuel race guerrier à shaman
					{
						dwSkillVnum = number(1, 111);
					}
					if ((randomTypeManuel >= 41 && randomTypeManuel <= 60) || randomTypeManuel == 2) // manuel race lycan
					{
						dwSkillVnum = number(170, 175);
					}
					if (item->GetOwner() != NULL)
					{
						if (((randomTypeManuel >= 61 && randomTypeManuel <= 80)  || randomTypeManuel == 3) && item->GetOwner()->GetLevel() >= 90) // manuel de parade de skill
						{
							dwSkillVnum = number(221, 229);
						}
						if (((randomTypeManuel >= 81 && randomTypeManuel <= 100)  || randomTypeManuel == 4) && item->GetOwner()->GetLevel() >= 90) // manuel de bonus de skill
						{
							dwSkillVnum = number(236, 244);
						}
					}
				...
				item->SetCount(randomNombreManuel);
			}
...
void ITEM_MANAGER::SaveSingleItem(LPITEM item)
{
	if (!item->GetOwner())
	{
		...
		//sys_err("ITEM_DELETE %s:%u", item->GetName(), dwID);
		return;
	}

	sys_log(0, "ITEM_SAVE %s:%d in %s window %d", item->GetName(), item->GetID(), item->GetOwner()->GetName(), item->GetWindow());
...
bool ITEM_MANAGER::CreateDropItem(LPCHARACTER pkChr, LPCHARACTER pkKiller, std::vector<LPITEM> & vec_item)
{
	...
	if (pkChr->IsStone())
	{
		...
		if (pkChr->GetLevel() < 40)
		{
			int rateNegatif = 0;
			if (pkChr->GetRaceNum() == 8115) // Lycan Metin
			{
				rateNegatif = 10;
			}
			if (pkKiller->GetLevel() > (pkChr->GetLevel() + 15))  // Réduction de le chance de drop
			{
				rateNegatif += (pkKiller->GetLevel() - pkChr->GetLevel() + 15);
			}
			if (pkKiller->GetLevel() < (pkChr->GetLevel() + 15) && pkChr->GetRaceNum() != 8115) // Augmentation de le chance de drop
			{
				rateNegatif -= ((pkChr->GetLevel() + 15) - pkKiller->GetLevel());
			}
			const INT METIN_STONE_ITEM_SPECIAL_NUM = 9;
			static DWORD c_adwMetinItemSpecial[METIN_STONE_ITEM_SPECIAL_NUM] =
			{
				39028, // 1 Objet enchanté IS
				39029, // 2 Objet de renforcement IS
				71151, // 3
				71152, // 4
				71084, // 5 Objet enchanté 
				71085, // 6 Objet de renforcement
				39030, // 7 Lecture concentrée
				39008, // 8 Parchemin d'exorcisme
				39006, // 9 Cape de bravoure
			};
			srand(time(0));
			int randomDropSpecialItem = rand() % METIN_STONE_ITEM_SPECIAL_NUM;
			srand(time(0));
			int randomChanceDropSpecialItem = rand() % 100 + 1;
			if (randomChanceDropSpecialItem < 100)
			{
				if (c_adwMetinItemSpecial[randomDropSpecialItem] == 39028 || c_adwMetinItemSpecial[randomDropSpecialItem] == 39029)
				{
					if (randomChanceDropSpecialItem >= (70 + rateNegatif))
					{
						srand(time(0));
						int randomNombre = rand() % 3 + 1;
						item = CreateItem(c_adwMetinItemSpecial[randomDropSpecialItem], 1, 0, true);
						item->SetCount(randomNombre);
						if (item) vec_item.push_back(item);
					}
				}
				else if (c_adwMetinItemSpecial[randomDropSpecialItem] == 77151 || c_adwMetinItemSpecial[randomDropSpecialItem] == 71152)
				{
					if (randomChanceDropSpecialItem >= (40 + rateNegatif))
					{
						srand(time(0));
						int randomNombre = rand() % 3 + 1;
						item = CreateItem(c_adwMetinItemSpecial[randomDropSpecialItem], 1, 0, true);
				...
				{
					if (randomChanceDropSpecialItem >= (60 + rateNegatif))
					{
						srand(time(0));
						int randomNombre = rand() % 3 + 1;
						item = CreateItem(c_adwMetinItemSpecial[randomDropSpecialItem], 1, 0, true);
		...
		else
		{
			...
			int rateNegatif = 0;
			if (pkKiller->GetLevel() > (pkChr->GetLevel() + 15))
			{
				rateNegatif += (pkKiller->GetLevel() - (pkChr->GetLevel() + 15)); // Réduction de le chance de drop
			}
			if (pkKiller->GetLevel() < (pkChr->GetLevel() + 15)) // Augmentation de le chance
			{
				rateNegatif -= ((pkChr->GetLevel() + 15) - pkKiller->GetLevel());
			}
			const INT METIN_STONE_ITEM_SPECIAL_NUM = 9;
			...
			if (randomChanceDropSpecialItem < 100)
			{
				if (c_adwMetinItemSpecial[randomDropSpecialItem] == 39028 || c_adwMetinItemSpecial[randomDropSpecialItem] == 39029)
				{
					if (randomChanceDropSpecialItem >= (65 + rateNegatif))
					{
						srand(time(0));
						int randomNombre = rand() % 5 + 1;
						item = CreateItem(c_adwMetinItemSpecial[randomDropSpecialItem], 1, 0, true);
						item->SetCount(randomNombre);
						if (item) vec_item.push_back(item);
					}
				...
				{
					if (randomChanceDropSpecialItem >= (35 + rateNegatif))
					{
						srand(time(0));
						int randomNombre = rand() % 5 + 1;
						item = CreateItem(c_adwMetinItemSpecial[randomDropSpecialItem], 1, 0, true);
				...
				{
					if (randomChanceDropSpecialItem >= (45 + rateNegatif))
					{
						srand(time(0));
						int randomNombre = rand() % 5 + 1;
						item = CreateItem(c_adwMetinItemSpecial[randomDropSpecialItem], 1, 0, true);
