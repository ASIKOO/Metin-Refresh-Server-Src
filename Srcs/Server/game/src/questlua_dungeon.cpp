...
namespace quest
{
	...
	int dungeon_drop(lua_State* L)
	{
		...
		if (pDungeon)
			pDungeon->Drop(lua_tonumber(L, 1));
		return 0;
	}
	...
	int dungeon_get_mob_stat(lua_State* L)
	{
		CQuestManager& q = CQuestManager::instance();
		LPDUNGEON pDungeon = q.GetCurrentDungeon();

		int typeStat = lua_tonumber(L,1);
		int vid = lua_tonumber(L,2);
		if (pDungeon)
		{
			if (CHARACTER_MANAGER::instance().Find(vid))
			{
				LPCHARACTER mob = CHARACTER_MANAGER::instance().Find(vid);
				switch(typeStat)
				{
					case 0: // HP
						lua_pushnumber(L, mob->GetHP());
					break;
				}
			}
		}
	...
	struct FAttaqueTarget
	{
		
		FAttaqueTarget(LPCHARACTER mob) : m_mob(mob)
		{
		...
		void operator()(LPENTITY ent)
		{
			if (ent->IsType(ENTITY_CHARACTER))
			{
				...
				{
					int attaqueDamage = 0;
					int randomDamage = number(1, 2);
					if (randomDamage == 1)
					{
						attaqueDamage = m_mob->GetMobDamageMin() * m_mob->GetMobDamageMultiply();
					}
					else
					{
						attaqueDamage = m_mob->GetMobDamageMax() * m_mob->GetMobDamageMultiply();
					}
					m_mob->Attack(ch, (BYTE)1);
					ch->SendDamagePacket(m_mob, attaqueDamage, (BYTE)1);
					ch->PointChange(POINT_HP, -attaqueDamage);
	...
	int dungeon_mob_attaque_special(lua_State* L)
	{
		CQuestManager& q = CQuestManager::instance();
		LPDUNGEON pDungeon = q.GetCurrentDungeon();
		int vid = lua_tonumber(L, 1);
		int idSkill = lua_tonumber(L, 2);
		if (pDungeon)
		{
			if (CHARACTER_MANAGER::instance().Find(vid))
			{
				...
				mob->SendMovePacket(FUNC_MOB_SKILL, idSkill, mob->GetX(), mob->GetY(), 0, timeNow);
				FAttaqueTarget f(mob);
	...
	int dungeon_mob_attaque_soin(lua_State* L)
	{
		CQuestManager& q = CQuestManager::instance();
		LPDUNGEON pDungeon = q.GetCurrentDungeon();
		int vidCible = lua_tonumber(L, 1);
		int vidExpediteur = lua_tonumber(L, 2);
		int pointHP = lua_tonumber(L, 3);
		if (pDungeon)
		{
			if (CHARACTER_MANAGER::instance().Find(vidCible))
			{
				LPCHARACTER cible = CHARACTER_MANAGER::instance().Find(vidCible);
				if (CHARACTER_MANAGER::instance().Find(vidExpediteur))
				{
					LPCHARACTER expediteur = CHARACTER_MANAGER::instance().Find(vidExpediteur);
					time_t timeNow = static_cast<time_t>(get_dword_time());
					expediteur->SendMovePacket(FUNC_ATTACK, 1, cible->GetX(), cible->GetY(), 0, timeNow);
					int hp = cible->GetHP() + pointHP;
					cible->PointChange(POINT_HP, hp);
					const char* effetSelectionTitre = "d:\\\\ymir work\\\\effect\\\\monster\\\\light_emissive1.mse";
					cible->SpecificEffectPacket(effetSelectionTitre);
					cible->UpdatePacket();
				}
			}
		}
	...
	int dungeon_kill_something(lua_State* L)
	{
		CQuestManager& q = CQuestManager::instance();
		LPDUNGEON pDungeon = q.GetCurrentDungeon();
	...
	int dungeon_set_mob_invincible(lua_State* L)
	{
		CQuestManager& q = CQuestManager::instance();
		LPDUNGEON pDungeon = q.GetCurrentDungeon();

		int vid = lua_tonumber(L,1);
		int statut = lua_tonumber(L,2);
		if (pDungeon)
		{
			if (CHARACTER_MANAGER::instance().Find(vid))
			{
				...
				if (statut == 1)
				{
					mob->m_invincibleStatut = true;
				}
				else
				{
					mob->m_invincibleStatut = false;
				}
			}
		}
		return 0;
	...
	void RegisterDungeonFunctionTable() 
	{
		...
		{
			{ "set_mob_invincible",		dungeon_set_mob_invincible	},
			{ "kill_something",		dungeon_kill_something		},
			{ "mob_attaque_soin",		dungeon_mob_attaque_soin	},
			{ "get_mob_stat",		dungeon_get_mob_stat		},
			{ "mob_attaque_special",	dungeon_mob_attaque_special	},
			{ "drop",			dungeon_drop		},
			{ "join",			dungeon_join		},
			{ "exit",			dungeon_exit		},
