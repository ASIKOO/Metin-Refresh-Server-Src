...
void LocaleService_TransferDefaultSetting()
{
	...
	if (SYSTEM_CHOIX_VNUM_SHOP > 0)
	{
		if (!ShopProto())
		{
			...
			fprintf(stdout, "Impossible de charger la liste des vnums de shop.");
			exit(1);
		}
	}
#endif

#ifdef ENABLE_DAILY_PRICE_SYSTEM
	if (SYSTEM_CARTE_CADEAU > 0)
	{
		if(!ChargementCadeau())
		{
			sys_err("Impossible de charger la liste des cadeaux.");
			fprintf(stdout, "Impossible de charger la liste des cadeaux.");
			exit(1);
		}
	}
#endif

#ifdef ENABLE_TITRE_SYSTEM
	if (SYSTEM_TITRE > 0)
	{
		if(!ChargementListeTitreMYSQL())
		{
			sys_err("Impossible de charger la configuration des titres depuis mysql!");
			fprintf(stdout, "Impossible de charger la configuration des titres depuis mysql!\n");
			exit(1);
		}
	}
#endif
	...
	if (g_stHostname != "auth")
	{
		if (!ChargementMeteo())
		{
			sys_err("Impossible de charger la table de météo depuis mysql!");
			fprintf(stdout, "Impossible de charger la table de météo depuis mysql!\n");
			exit(1);
		}
	}
#endif
	...
	if (g_stHostname != "auth")
	{
		...
		{
			sys_err("Impossible de charger la table d'environnement depuis mysql!");
	...
	if (g_stHostname != "auth")
	{
		if (!ChargementListeMobScaleMYSQL())
		{
			sys_err("Impossible de charger la table de scale depuis mysql!");
			fprintf(stdout, "Impossible de charger la table de scale depuis mysql!\n");
	...
	if (g_stHostname != "auth")
	{
		if (!LectureListeBlocageObjet())
		{
			sys_err("Impossible de charger la table de blocage objet depuis mysql!");
			fprintf(stdout, "Impossible de charger la table de blocage depuis mysql!\n");
...
bool LC_IsEurope()
{
	...
	switch ((int) val)
	{
		...
		case LC_NETHERLANDS:
		case LC_USA:
		case LC_WE_KOREA:	// ÇÑ±¹ÀÌÁö¸¸ UK ¹öÀü ±â¹ÝÀÌ¹Ç·Î ¿©±â ³ÖÀ½
		case LC_TAIWAN:		// ´ë¸¸ÀÌÁö¸¸ WE_KOREA ¹öÀü ±â¹ÝÀÌ¹Ç·Î ¿©±â ³ÖÀ½
