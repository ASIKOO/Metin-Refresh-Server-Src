...
ACMD(do_rain)
{
	...
	if (0 == arg1[0])
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Utilisation: rain active pour activer");
		ch->ChatPacket(CHAT_TYPE_INFO, "	     rain desactive pour désactiver");
		return;
	}
	...
	for (itertype(c_ref_set) it = c_ref_set.begin(); it != c_ref_set.end(); ++it)
	{
		...
		ch->ChatPacket(CHAT_TYPE_COMMAND, "rain %s", strArg1.c_str());
	}
}
...
struct FuncPurge
{
	...
	void operator () (LPENTITY ent)
	{
		...
		if (std::find(liste_vnum_shop_vector.begin(), liste_vnum_shop_vector.end(),pkChr->GetRaceNum())!=liste_vnum_shop_vector.end() == false)
		{
...
ACMD(do_state)
{
	...
	ch->ChatPacket(CHAT_TYPE_INFO, "   ATT:%3d%% DEF:%3d%% EXP:%3d%% ITEMx%d GOLDx%lld",
	...
	if (*arg1)
	{
		switch (LOWER(*arg1))
		{
			...
			case 'c':	// cube
				...
				break;
			case 'd':
				ch->ChatPacket(CHAT_TYPE_INFO, "Rechargement des types de magasins.");
				ShopProto();
				sys_log(0, "Rechargement des types de magasins.");
				break;
			case 'e':
				Acce_init();
				break;
			case 'f':
				...
				break;
			case 'g':
				LectureListeBlocageObjet();
				break;

				//RELOAD_ADMIN

				//END_RELOAD_ADMIN
			case 'm': // Système météo.
				ch->ChatPacket(CHAT_TYPE_INFO, "Rechargement de la météo en cours..");
				if (ChargementMeteo())
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "Rechargement de la météo réussi.");
				}
				else
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "Rechargement de la météo échoué.");
				}
				break;
			case 'n': // Système environnement.
				ch->ChatPacket(CHAT_TYPE_INFO, "Rechargement des environnements en cours..");
				if (ChargementEnvironnement())
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "Rechargement des environnements réussi.");
				}
				else
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "Rechargement des environnements échoué.");
				}
				break;
			case 'p':
			...
			case 's':
				...
				break;
			case 't': // Système titre.
				if (SYSTEM_TITRE > 0)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "Rechargement des configurations du système de titre..");
					if(ChargementListeTitreMYSQL())
					{
						const DESC_MANAGER::DESC_SET & c_ref_set = DESC_MANAGER::instance().GetClientSet();

						for (itertype(c_ref_set) it = c_ref_set.begin(); it != c_ref_set.end(); ++it)
						{
							LPCHARACTER chPVP = (*it)->GetCharacter();

							if (!chPVP)
								continue;

							if(chPVP->m_titreSystem != NULL)
							{
								chPVP->m_titreSystem->ChargementTitre(chPVP);
							}
						}
						ch->ChatPacket(CHAT_TYPE_INFO, "Rechargement des configurations du système de titre réussi !");
					}
					else
					{
						ch->ChatPacket(CHAT_TYPE_INFO, "Rechargement des configurations du système de titre échoué !");
					}
				}
				break;
			case 'u':
				...
				LoadStateUserCount();
				break;
			case 'w': // Système mob_scale
				ch->ChatPacket(CHAT_TYPE_INFO, "Rechargement des configurations du système de scale..");
				ChargementListeMobScaleMYSQL();
				const DESC_MANAGER::DESC_SET & c_ref_set = DESC_MANAGER::instance().GetClientSet();
		...
		case 'c':
			{
				...
				if (test_server || GMLevel == GM_PLAYER || GMLevel == GM_IMPLEMENTOR)
					// °Ç¼³ Àç·á ¼Ò¸ðÇÏ±â (Å×¼·¿¡¼­´Â GMµµ ¼Ò¸ð)
				{
					// °Ç¼³ ºñ¿ë ¼Ò¸ð
					ch->PointChange(POINT_GOLD, -t->dwPrice, false);
	...
	case JOB_SURA:
		{
			...
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(17569 );
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
	...
	case JOB_ASSASSIN:
		{
			...
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(2379);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
