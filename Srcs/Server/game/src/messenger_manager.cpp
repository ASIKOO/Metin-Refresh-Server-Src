...
void MessengerManager::RequestToAdd(LPCHARACTER ch, LPCHARACTER target)
{
	...
	{
	    ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("ªÛ¥ÎπÊ¿Ã ƒ£±∏ √ﬂ∞°∏¶ πﬁ¿ª ºˆ æ¯¥¬ ªÛ≈¬¿‘¥œ¥Ÿ."));
	    return;
	}
...
void MessengerManager::__AddToList(MessengerManager::keyA account, MessengerManager::keyA companion)
{
	...
	if (d)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<∏ﬁΩ≈¡Æ> %s ¥‘¿ª ƒ£±∏∑Œ √ﬂ∞°«œø¥Ω¿¥œ¥Ÿ."), companion.c_str());
	}
...
void MessengerManager::__RemoveFromList(MessengerManager::keyA account, MessengerManager::keyA companion)
{
	...
	if (d)
		ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<∏ﬁΩ≈¡Æ> %s ¥‘¿ª ∏ﬁΩ≈¿˙ø°º≠ ªË¡¶«œø¥Ω¿¥œ¥Ÿ."), companion.c_str());
}
...
void MessengerManager::RemoveFromList(MessengerManager::keyA account, MessengerManager::keyA companion)
{
	...
	if (m_Relation[account].find(companion) == m_Relation[account].end() || m_InverseRelation[companion].find(account) == m_InverseRelation[companion].end())
	{
		...
		if (ch)
		{
			sys_err("MessengerManager::RemoveFromList: %s essai d'executer une injection sql", ch->GetName());
			DBManager::Instance().DirectQuery("UPDATE account_alpha.account SET status = 'BAN' WHERE id = %u", ch->GetAID());
			if (ch->GetDesc())
				ch->GetDesc()->DelayedDisconnect(3);
		}
		else
			sys_err("MessengerManager::RemoveFromList: Omg! Un fantÙme essai d'utiliser cette fonction!");
		return;
	}
