...
class CMonarch : public singleton<CMonarch>
{
	public:
		...
		int HealMyEmpire(LPCHARACTER ch, long long price);
		void SetMonarchInfo(TMonarchInfo * pInfo);

		bool IsMonarch(DWORD pid, BYTE bEmpire);
		bool IsMoneyOk(long long price, BYTE bEmpire);
		bool SendtoDBAddMoney(long long Money, BYTE bEmpire, LPCHARACTER ch);
		bool SendtoDBDecMoney(long long Money, BYTE bEmpire, LPCHARACTER ch);

		bool AddMoney(long long Money, BYTE bEmpire);
		bool DecMoney(long long Money, BYTE bEmpire);
		long long GetMoney(BYTE bEmpire);
