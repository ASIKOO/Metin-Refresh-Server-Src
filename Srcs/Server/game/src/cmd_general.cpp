...
{ 
	...
	if (!*arg1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO,"Invalid Command Exemple: /channel <ch>"); 
		return;
	} 
	DWORD id; 
	if (atoi(arg1))
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Vous ne pouvez pas vous changer de channel dans un donjon.");
			return;
		}
	}
}
...
//		case POINT_EXTRACT_HP_PCT:	return LC_TEXT("");
		...
		case POINT_ACCE_ABSORB:		return "Taux d'absorbation de %d%%";
		case POINT_RESIST_MAGIC_REDUCTION:	return "Effet d'anti-magie de +%d%%";
		default:					return NULL;
	}
}
...
ACMD(do_PetSkill)
{
	...
	if (ch->GetNewPetSystem()->IsActivePet())
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "Vous devez invoquer votre animal de compagnie.");
}
...
ACMD(do_FeedCubePet) 
{
	...
	if (ch->GetNewPetSystem()->IsActivePet())
		...
		ch->ChatPacket(CHAT_TYPE_INFO, "Vous n'avez pas invoquer votre animal.");
}
...
ACMD(do_PetEvo) 
{
	...
	if (ch->GetExchange() || ch->GetMyShop() || ch->GetShopOwner() || ch->IsOpenSafebox() || ch->IsCubeOpen()) {
		ch->ChatPacket(CHAT_TYPE_INFO, "[PetEvolution] Vous ne nouvez pas faire évoluer votre animal maintenant !");
		return;
	}
	if (ch->GetNewPetSystem()->IsActivePet()) {
		...
		if (ch->GetNewPetSystem()->GetLevel() == 40 && tmpevo == 0 ||
			...
			for (int b = 0; b < 1; b++) {
				if (ch->CountSpecifyItem(it[tmpevo][b]) < ic[tmpevo][b]) {
					ch->ChatPacket(CHAT_TYPE_INFO, "[PetEvolution] Objet requis:");
					for (int c = 0; c < 1; c++) {
			...
			ch->ChatPacket(CHAT_TYPE_INFO, "Vous ne pouvez pas faire évoluer votre animal !");
			return;
		}

	}else
		ch->ChatPacket(CHAT_TYPE_INFO, "Votre animal doit être invoqué !");
...
ACMD(do_selectprivateshop)
{
	if (SYSTEM_CHOIX_VNUM_SHOP <= 0)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Le système n'est pas activé");
		return;
	}
	const char *line;
	char arg1[256], arg2[256], arg3[256];
	line = two_arguments(argument, arg1, sizeof(arg1), arg2, sizeof(arg2));
	one_argument(line, arg3, sizeof(arg3));
	const std::string& strArgument1 = std::string(arg1);
	const std::string& strArgument2 = std::string(arg2);
	const std::string& strArgument3 = std::string(arg3);
	if (strArgument1 != "")
	{
		if (strArgument1 == "SELECT_SHOP_VNUM")
		{
			
			if (atoi(strArgument2.c_str()))
			{
				int vnumShopTemp = atoi(strArgument2.c_str());
				bool vnumExistant = false;
				for(int i = 0; i < liste_vnum_shop_vector.size(); i++)
				{
					if (vnumShopTemp == liste_vnum_shop_vector[i])
					{
						vnumExistant = true;
					}
				}
				if (vnumExistant)
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "Le type de magasin a bien été choisi.");
					ch->ChatPacket(CHAT_TYPE_COMMAND, "CloseSelectPrivateShop");
					ch->ChatPacket(CHAT_TYPE_COMMAND, "OpenPrivateShop");
					ch->VnumShop = vnumShopTemp;
				}
				else
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "Ce type de magasin n'existe pas !");
					return;
				}
			}
			else
			{
				ch->ChatPacket(CHAT_TYPE_INFO, "L'argument est incorrecte.");
				return;
			}
		}
		else if (strArgument1 == "OPEN_SHOP_VNUM")
		{
			for (int i = 0; i < liste_vnum_shop_vector.size(); i++)
			{
				ch->ChatPacket(CHAT_TYPE_COMMAND, "SelectPrivateShopNPC %d-%s", liste_vnum_shop_vector[i], liste_nom_shop_vector[i].c_str());
			} 
		}
		else
		{
...
ACMD(do_enchantement_plus)
{
	...
	one_argument(line, arg3, sizeof(arg3));
	const std::string& strArg1 = std::string(arg1);
	const std::string& strPosition = std::string(arg2);
	const std::string& strChoix = std::string(arg3);
	int positionObjet = 0;
	int choix = 0;

	positionObjet = atoi(strPosition.c_str());
	
	choix = atoi(strChoix.c_str());
	switch (LOWER(arg1[0]))
	{
		case 't':
			if (ch->TestEnchantementPlus)
			{
				ch->ChatPacket(CHAT_TYPE_INFO, "Veuillez choisir vos bonus.");
				return;
			}
			else // Envoi les anciens et nouveaux bonus de l'objet.
			{
				LPITEM item = ch->GetInventoryItem(positionObjet);
				if (item != NULL)
				{
					if (item->GetType() == ITEM_ARMOR || item->GetType() == ITEM_WEAPON)
					{
						if (ch->CountSpecifyItem(71189) <= 0)
						{
							ch->ChatPacket(CHAT_TYPE_INFO, "Vous n'avez aucun objet de changement de bonus+.");
							return;
						}
						ch->RemoveSpecifyItem(71189, 1);
						if (item->GetAttributeCount() <= 0)
						{
							ch->ChatPacket(CHAT_TYPE_INFO, "Votre objet ne possède aucun bonus.");
							return;
						}

						ch->TypeAncienBonusEnchantement0 = item->GetAttributeType(0);
						ch->ValeurAncienBonusEnchantement0 = item->GetAttributeValue(0);

						ch->TypeAncienBonusEnchantement1 = item->GetAttributeType(1);
						ch->ValeurAncienBonusEnchantement1 = item->GetAttributeValue(1);

						ch->TypeAncienBonusEnchantement2 = item->GetAttributeType(2);
						ch->ValeurAncienBonusEnchantement2 = item->GetAttributeValue(2);

						ch->TypeAncienBonusEnchantement3 = item->GetAttributeType(3);
						ch->ValeurAncienBonusEnchantement3 = item->GetAttributeValue(3);

						ch->TypeAncienBonusEnchantement4 = item->GetAttributeType(4);
						ch->ValeurAncienBonusEnchantement4 = item->GetAttributeValue(4);


						item->ChangeAttribute();

						ch->TypeBonusEnchantement0 = item->GetAttributeType(0);
						ch->ValeurBonusEnchantement0 = item->GetAttributeValue(0);
						ch->TypeBonusEnchantement1 = item->GetAttributeType(1);
						ch->ValeurBonusEnchantement1 = item->GetAttributeValue(1);
						ch->TypeBonusEnchantement2 = item->GetAttributeType(2);
						ch->ValeurBonusEnchantement2 = item->GetAttributeValue(2);
						ch->TypeBonusEnchantement3 = item->GetAttributeType(3);
						ch->ValeurBonusEnchantement3 = item->GetAttributeValue(3);
						ch->TypeBonusEnchantement4 = item->GetAttributeType(4);
						ch->ValeurBonusEnchantement4 = item->GetAttributeValue(4);

						item->SetForceAttribute(0, ch->TypeAncienBonusEnchantement0, ch->ValeurAncienBonusEnchantement0);
						item->SetForceAttribute(1, ch->TypeAncienBonusEnchantement1, ch->ValeurAncienBonusEnchantement1);
						item->SetForceAttribute(2, ch->TypeAncienBonusEnchantement2, ch->ValeurAncienBonusEnchantement2);
						item->SetForceAttribute(3, ch->TypeAncienBonusEnchantement3, ch->ValeurAncienBonusEnchantement3);
						item->SetForceAttribute(4, ch->TypeAncienBonusEnchantement4, ch->ValeurAncienBonusEnchantement4);


						ch->ChatPacket(CHAT_TYPE_COMMAND, "ReceptionEnchantPlus _%d_%d_%d_%d_%d_%d_%d_%d_%d_%d", ch->TypeBonusEnchantement0, ch->ValeurBonusEnchantement0, ch->TypeBonusEnchantement1, ch->ValeurBonusEnchantement1,  ch->TypeBonusEnchantement2, ch->ValeurBonusEnchantement2,  ch->TypeBonusEnchantement3, ch->ValeurBonusEnchantement3, ch->TypeBonusEnchantement4, ch->ValeurBonusEnchantement4);
						ch->TestEnchantementPlus = true;
					}
					else
					{
						ch->ChatPacket(CHAT_TYPE_INFO, "Vous ne pouvez pas changer les bonus de cet objet.");
						return;
					}
				}
				else
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "Cet objet est invalide.");
					return;
				}
			}
		break;
						...
						if (item->GetType() == ITEM_ARMOR || item->GetType() == ITEM_WEAPON)
						{
							switch(choix)
							{
								case 1: // ancien bonus
									...
									item->SetForceAttribute(3, ch->TypeAncienBonusEnchantement3, ch->ValeurAncienBonusEnchantement3);
									item->SetForceAttribute(4, ch->TypeAncienBonusEnchantement4, ch->ValeurAncienBonusEnchantement4);
									ch->ChatPacket(CHAT_TYPE_INFO, "Vos bonus ont été conservé sur votre objet.");
									ch->ChatPacket(CHAT_TYPE_COMMAND, "CloseEnchantItemPlusWindow");
								break;
								case 2: // nouveau bonus
									item->SetForceAttribute(0, ch->TypeBonusEnchantement0, ch->ValeurBonusEnchantement0);
									item->SetForceAttribute(1, ch->TypeBonusEnchantement1, ch->ValeurBonusEnchantement1);
									item->SetForceAttribute(2, ch->TypeBonusEnchantement2, ch->ValeurBonusEnchantement2);
									item->SetForceAttribute(3, ch->TypeBonusEnchantement3, ch->ValeurBonusEnchantement3);
									item->SetForceAttribute(4, ch->TypeBonusEnchantement4, ch->ValeurBonusEnchantement4);
									ch->ChatPacket(CHAT_TYPE_INFO, "Vos bonus ont été changé sur votre objet.");
									ch->ChatPacket(CHAT_TYPE_COMMAND, "CloseEnchantItemPlusWindow");
								break;
							}
							ch->TestEnchantementPlus = false;
						}
						else
						{
							ch->ChatPacket(CHAT_TYPE_INFO, "Vous ne pouvez pas changer les bonus de cet objet.");
...
ACMD(do_hauteur)
{
	...
	const std::string& strArg3 = std::string(arg3);
	if (0 == arg1[0])
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Utilisation: hauteur val");
		return;
	}

	if (ch->m_nouveauMouvementSystem == NULL)
	{
		return;
	}
	...
	if(strArg1 == "v")
	{
		...
		ss >> valeur;
		ss.clear();
		ch->m_nouveauMouvementSystem->hauteurZ = valeur;
	}
	else if (strArg1 == "s")
	{
		float valeur = 0;
		...
		ss >> valeur;
		ss.clear();
		ch->m_nouveauMouvementSystem->hauteurSol = valeur;
	}
	else if (strArg1 == "n")
	{
		if (strArg2 == "on")
		{
			ch->m_nouveauMouvementSystem->IsNage = true;
		}
		else
		{
			ch->m_nouveauMouvementSystem->IsNage = false;
		}
	}
	else if (strArg1 == "o")
	{
...
ACMD(do_fly)
{
	...
	if (0 == arg1[0])
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Utilisation: fly start");
		ch->ChatPacket(CHAT_TYPE_INFO, "	     fly off");
		return;
	}
	...
	{
		case 's':
			ch->m_nouveauMouvementSystem->IsVol = true;
		break;
		...
		case 'o':
			...
			{
				ch->ChatPacket(CHAT_TYPE_COMMAND, "TitreMessage %s", "Vous-ne-pouvez-pas-attérir-ici.");
				return;
			}
			else
			{
...
ACMD(do_titre)
{
	if (SYSTEM_TITRE <= 0)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Le système de titre pvp n'est pas activé.");
		ch->ChatPacket(CHAT_TYPE_COMMAND, "%s", "CloseTitreInterface");
		return;
	}

	if (ch->m_titreSystem == NULL)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Le système de titre n'est pas initialisé sur votre personnage.");
		ch->ChatPacket(CHAT_TYPE_INFO, "Contactez l'équipe en cas de problème.");
		ch->ChatPacket(CHAT_TYPE_COMMAND, "%s", "CloseTitreInterface");
		return;
	}
	const char *line;
	...
	one_argument(line, arg3, sizeof(arg3));
	const std::string& strArg1 = std::string(arg1);
	const std::string& strNomTitre = std::string(arg2);
	if (0 == arg1[0])
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Utilisation: titre open");
		ch->ChatPacket(CHAT_TYPE_INFO, "	     titre select");
		ch->ChatPacket(CHAT_TYPE_INFO, "	     titre afk");
		return;
	}
	...
	switch (LOWER(arg1[0]))
	{
		case 'o':
			if (ch->m_titreSystem != NULL)
			{
				ch->m_titreSystem->UpdateInterface(ch);
			}
			else
			{
				return;
		...
		case 's':
			if (ch->m_titreSystem != NULL)
			{
				ch->m_titreSystem->SelectTitrePVP(ch, strNomTitre);
			}
			else
			{
				return;
		...
		case 'a':
			if (ch->m_titreSystem != NULL)
			{
				if ((ch->m_titreSystem->ActiveTempsAfk+ch->m_titreSystem->TempsActiveTempsAfk) < get_global_time())
				{
					if (ch->m_titreSystem->AFK == false)
					{
						ch->m_titreSystem->AFK = true;
						ch->last_X = ch->GetX();
						ch->last_Y = ch->GetY();
						ch->UpdatePacket();
						std::string Message = "Vous êtes à présent AFK.";
						replace(Message.begin(), Message.end(), ' ', '-');
						ch->ChatPacket(CHAT_TYPE_COMMAND, "TitreMessage %s", Message.c_str());
					}
					else
					{
						ch->m_titreSystem->AFK = false;
						ch->last_X = ch->GetX()+1;
						ch->last_Y = ch->GetY()+1;
						ch->UpdatePacket();
						std::string Message = "Vous n'êtes plus AFK.";
						replace(Message.begin(), Message.end(), ' ', '-');
						ch->ChatPacket(CHAT_TYPE_COMMAND, "TitreMessage %s", Message.c_str());
					}
					ch->m_titreSystem->ActiveTempsAfk = get_global_time();
				}
				else
				{
					std::ostringstream oss;
					oss << "Veuillez patienter ";
					oss << ((ch->m_titreSystem->ActiveTempsAfk+ch->m_titreSystem->TempsActiveTempsAfk)-get_global_time());
					oss << " seconde(s).";
...
ACMD(do_cartecadeau)
{
	if (SYSTEM_CARTE_CADEAU <= 0)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Le système de carte cadeau n'est pas activé.");
		return;
	}
	if (!ch->m_CadeauSystem)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Le système de carte cadeau n'est pas initialisé sur votre personnage.");
		ch->ChatPacket(CHAT_TYPE_INFO, "Contactez l'équipe en cas de problème.");
		return;
	}
	const char *line;
	...
	if (strArgument1 != "")
	{
		if (strArgument1 == "SELECT_CARTE_ID")
		{
			
			if (atoi(strArgument2.c_str()))
			{
				int carteIdTemp = atoi(strArgument2.c_str()) - 1;
				ch->m_CadeauSystem->ReceptionCadeau(carteIdTemp, ch);
			}
			else
			{
				ch->ChatPacket(CHAT_TYPE_INFO, "L'argument est incorrecte.");
...
ACMD(do_magasinier)
{
	if (ch->m_magasinierSystem == NULL)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Le système de magasinier n'est pas initialisé sur votre personnage.");
		ch->ChatPacket(CHAT_TYPE_INFO, "Contactez l'équipe en cas de problème.");
		return;
	}
	const char *line;
	...
	const std::string& strArgument2 = std::string(arg2);
	const std::string& strArgument3 = std::string(arg3);
	const std::string& strArgument4 = std::string(arg4);
	if (strArgument1 != "")
	{

		if (strArgument1 == "OPEN")
		{
			if (atoi(strArgument2.c_str()))
			{
				int type = atoi(strArgument2.c_str());
				std::string pseudo;
				if(strArgument3 != "");
				{
					pseudo = strArgument3;
				}
				ch->m_magasinierSystem->Open(ch, type, pseudo);
			}
			else
			{
				return;
		...
		if(strArgument1 == "ADD_ITEM")
		{
			if(atoi(strArgument2.c_str()))
			{
				int pos = atoi(strArgument2.c_str());
				if (atoi(strArgument3.c_str()))
				{
					int type = atoi(strArgument3.c_str());
					if(atoi(strArgument4.c_str()))
					{
						int posDest = atoi(strArgument4.c_str());
						ch->m_magasinierSystem->AddMagasinier(ch, pos, type, posDest);
					}
					else
					{
						return;
		...
		if(strArgument1 == "GET_ITEM")
		{
			if(atoi(strArgument2.c_str()))
			{
				...
				if (atoi(strArgument3.c_str()))
				{
					...
					{
						int posDest = atoi(strArgument4.c_str());
						ch->m_magasinierSystem->GetMagasinier(ch, pos, type, posDest);
					}
				}
				else
				{
...
ACMD(do_remove_shop)
{
	...
	if (strArgument1 != "")
	{
		char tempShopNom[64+1];
		DBManager::instance().EscapeString(tempShopNom, sizeof(tempShopNom), strArgument1.c_str(), strArgument1.length());
		std::string titreShop = tempShopNom;
		replace(titreShop.begin(), titreShop.end(), '-', ' ');
		int MapIndex = ch->GetMapIndex();
		int Channel = g_bChannel;
		int VidSHOP = 0;
		LPCHARACTER shopNpc;

		SQLMsg * pMsgTableShop = DBManager::instance().DirectQuery("SELECT * from private_herra_shops%s WHERE title='%s' AND status='1' AND channel='%d' AND map_index='%d';", get_table_postfix(), titreShop.c_str(), Channel, MapIndex);
		if (pMsgTableShop->Get()->uiNumRows > 0)
		{
			MYSQL_ROW rowShop;
			
			rowShop = mysql_fetch_row(pMsgTableShop->Get()->pSQLResult);
			str_to_number(VidSHOP, rowShop[1]);
			if (CHARACTER_MANAGER::instance().Find(VidSHOP))
			{
				shopNpc = CHARACTER_MANAGER::instance().Find(VidSHOP);
				DBManager::instance().DirectQuery("UPDATE private_herra_shops%s SET time = UNIX_TIMESTAMP(), status = '2' WHERE shop_id = '%d' AND title = '%s';", get_table_postfix(), VidSHOP, titreShop.c_str());
				shopNpc->CloseHerraShop();
				ch->ChatPacket(CHAT_TYPE_INFO, "Le shop %s a bien été fermé.", titreShop.c_str());
			}
		}
		else
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Le shop %s est introuvable", titreShop.c_str());
		}
		delete pMsgTableShop;
	}
	else
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Veuillez saisir le nom du shop");
	}
}
...
ACMD(do_offlineshop)
{
	if (ch->m_offlineShopSystem == NULL)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Le système de shop offline n'est pas initialisé sur votre personnage.");
		ch->ChatPacket(CHAT_TYPE_INFO, "Contactez l'équipe en cas de problème.");
		return;
	}
	...
	const std::string& strArgument2 = std::string(arg2);
	const std::string& strArgument3 = std::string(arg3);
	//ch->ChatPacket(CHAT_TYPE_INFO, "DEBUGFILES PHOne: requête shop: %s, %s, %s", strArgument1.c_str(), strArgument2.c_str(), strArgument3.c_str());
	if (strArgument1 != "")
	{
		if (strArgument1 != "REFRESH" && strArgument1 != "OPEN")
		{
			if (atoi(strArgument2.c_str()))
			{
				if (strArgument1 == "SELECT_SHOP")
				{
					int tempId = atoi(strArgument2.c_str());
					ch->m_offlineShopSystem->RefreshItem(ch, tempId);
				}
				if (strArgument1 == "WITH_DRAW")
				{
					int tempId = atoi(strArgument2.c_str());
					ch->m_offlineShopSystem->GetYang(ch, tempId);
				}
				if (strArgument1 == "GET_WON")
				{
					int tempId = atoi(strArgument2.c_str());
					ch->m_offlineShopSystem->GetWon(ch, tempId);
				}
				if (strArgument1 == "GET_ITEM")
				{
					if (atoi(strArgument2.c_str()))
					{
						...
						if (atoi(strArgument3.c_str()))
						{
							int tempPos = atoi(strArgument3.c_str());
							ch->m_offlineShopSystem->GetItem(ch, tempId, tempPos);
						}
						else
						{
							ch->m_offlineShopSystem->GetItem(ch, tempId, 0);
						}
					}
				}
				if (strArgument1 == "CLOSE_SHOP")
				{
					if (atoi(strArgument2.c_str()))
					{
						int tempId = atoi(strArgument2.c_str());
						ch->m_offlineShopSystem->FermerShop(ch, tempId);
					}
				}
			}
			else
			{
				ch->ChatPacket(CHAT_TYPE_INFO, "Donnée envoyé incorrecte.");
			}
		}
		else // Demande de refresh.
		{
			if (strArgument1 == "REFRESH")
			{
				ch->m_offlineShopSystem->RefreshShop(ch);
			}
			if (strArgument1 == "OPEN")
			{
				ch->m_offlineShopSystem->OpenShop(ch);
			}
		}
	}
	else
...
ACMD(do_in_game_mall)
{
	...
	if (LC_IsEurope() == true)
	{
		...
		snprintf(buf, sizeof(buf), "mall https://%s/index.php?pid=%u&c=%s&sid=%d&sas=%s",
...
ACMD(do_transmutate)
{
	...
	if (atoi(arg1) && atoi(arg2))
	{
		...
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Vous avez besoin des objets pour faire la transmutation.");
			return;
		}
		else
		{
...
#ifdef ENABLE_POPUP_SYSTEM
					ch->ChatPacket(CHAT_TYPE_COMMAND, "TitreMessage Votre-transmutation-a-été-effectué-avec-succès.");
#endif
					ch->ChatPacket(CHAT_TYPE_INFO, "Le second objet a été détruit lors de la transmutation.");
					ch->ChatPacket(CHAT_TYPE_INFO, "50000000 de yangs ont été retiré lors de la transmutation.");
				}
				else
				{
					ch->ChatPacket(CHAT_TYPE_INFO, "Vous n'avez pas assez de yang pour faire la transmutation.");
				}
			}
			else
			{
				ch->ChatPacket(CHAT_TYPE_INFO, "Le type des objets ne correspondent pas.");
			}
		}
	}
	else
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Les arguments sont incorrectes.");
	}
}
#endif
