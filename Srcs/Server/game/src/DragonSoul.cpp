...
bool DSManager::DoRefineGrade(LPCHARACTER ch, TItemPos (&aItemPoses)[DRAGON_SOUL_REFINE_GRID_SIZE])
{
	...
	if (ch->GetGold() < (long long int)fee)
	{
...
bool DSManager::ActivateDragonSoul(LPITEM pItem)
{
	...
	if (inventorysize + WEAR_MAX_NUM + DS_SLOT_MAX * deck_idx <= pItem->GetCell() &&
			pItem->GetCell() < inventorysize + WEAR_MAX_NUM + DS_SLOT_MAX * (deck_idx + 1))
	{
