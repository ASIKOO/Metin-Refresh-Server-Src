...
void VerifLicenceSoftYou(string access) {
  ...
  // memset(&serv_addr, '\0', sizeof(serv_addr)); 
    ...
    std::string message;
	message = "GameServerMetin2France-"+access+"-"+dateHerra+"-FreeBSD9.2";
...
//#ifdef ENABLE_CHECK_LICENCE_GAME
	...
	string read_file_name("/home/game/share/pinghostingame");
...
//#endif
	...
	std::ifstream exppet_table_open("/home/game/share/exppettable.txt");
	...
	while (!exppet_table_open.eof())
	{
		...
		if (exppet_table_common[exppet_table_counter] < 2147483647) {
			sys_log(0, "Chargement table de pet d'exp exppettable.txt: %d !", exppet_table_common[exppet_table_counter]);
			exppet_table_counter++;
		}
		else {
			fprintf(stderr, "[main] Impossible de lire la table d'exp de pet\n");
			break;
		}
	}
#endif
