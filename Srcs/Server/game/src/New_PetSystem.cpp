...
bool CNewPetActor::IncreasePetSkill(int skill) {
	...
	for (int i = 0; i < 3; ++i) { //Itero gli slot per cercare la skill
		if (m_dwskillslot[i] == skill) {  //Se trova la skill o la aumenta oppure e' gi?max
			if (m_dwskill[i] < 20) {
				m_dwskill[i] += 1;
				m_pkOwner->ChatPacket(CHAT_TYPE_INFO, "Votre Skill a atteint le niveau: %d", m_dwskill[i]);
				m_pkOwner->ChatPacket(CHAT_TYPE_COMMAND, "PetSkill %d %d %d", i, m_dwskillslot[i], m_dwskill[i]);
				return true;
			}
			else {
				m_pkOwner->ChatPacket(CHAT_TYPE_INFO, "Votre Skill a atteint le niveau maximum !");
				return false;
			}
		}
	}
	...
	for (int i = 0; i < 3; ++i) {
		if (m_dwskillslot[i] == 0 || m_dwskillslot[i] == -1) { 
			...
			m_dwskill[i] = 1;
			m_pkOwner->ChatPacket(CHAT_TYPE_INFO, "Votre animal a appris une nouvelle compétence!");
			m_pkOwner->ChatPacket(CHAT_TYPE_COMMAND, "PetSkill %d %d %d", i, m_dwskillslot[i], m_dwskill[i]);
			return true;
		}
	}
	...
	*/
	m_pkOwner->ChatPacket(CHAT_TYPE_INFO, "Votre animal de compagnie n'est pas en mesure d'apprendre d'autres compétences!");
	return false;
}
...
bool CNewPetActor::IncreasePetEvolution() {
	if (m_dwevolution < 3) {
		...
		m_pkOwner->ChatPacket(CHAT_TYPE_INFO, "Votre animal a atteint l'évolution maximale !");
		return false;
	}
	return true;
}
...
void CNewPetActor::DoPetSkill(int skillslot) {
	...
	case 10:
	{
		if (get_global_time() - m_pkOwner->GetNewPetSkillCD(0) <= 480) {
			m_pkOwner->ChatPacket(CHAT_TYPE_INFO, "Capacité de recharge %ds restante!", 480 - (get_global_time() - m_pkOwner->GetNewPetSkillCD(0)));
			return;
		}
		if (m_pkOwner->GetHPPct() > 20) {
			m_pkOwner->ChatPacket(CHAT_TYPE_INFO, "Vous pouvez utiliser cette compétence uniquement avec PV <= 20%!");
			return;
		}
	...
	case 17:
	{
		if (get_global_time() - m_pkOwner->GetNewPetSkillCD(1) <= 600) {
			m_pkOwner->ChatPacket(CHAT_TYPE_INFO, "Capacité de recharge %ds restante!", 600 - (get_global_time() - m_pkOwner->GetNewPetSkillCD(1)));
			return;
		}
		m_pkOwner->SetNewPetSkillCD(1, get_global_time());
		m_pkOwner->ChatPacket(CHAT_TYPE_INFO, "Vous êtes immortel!");
	...
	case 18:
	{
		if (get_global_time() - m_pkOwner->GetNewPetSkillCD(2) <= 480) {
			m_pkOwner->ChatPacket(CHAT_TYPE_INFO, "Capacité de recharge %ds restante!", 480 -(get_global_time() - m_pkOwner->GetNewPetSkillCD(2)));
			return;
		}
