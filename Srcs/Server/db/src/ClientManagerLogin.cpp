...
void CClientManager::RESULT_LOGIN_BY_KEY(CPeer * peer, SQLMsg * msg)
{
	...
	{
		snprintf(szQuery, sizeof(szQuery), "SELECT id, name, job, level, alignment, st, ht, dx, iq, part_main, part_hair, part_acce, x, y, skill_group, change_name, 7emeSkill, 8emeSkill, cheque, titre FROM player%s WHERE account_id=%u", GetTablePostfix(), info->pAccountTable->id);
	}
	else
	{
		snprintf(szQuery, sizeof(szQuery), "SELECT id, name, job, level, playtime, st, ht, dx, iq, part_main, part_hair, part_acce, x, y, skill_group, change_name, 7emeSkill, 8emeSkill, cheque, titre FROM player%s WHERE account_id=%u", GetTablePostfix(), info->pAccountTable->id);
	}
		...
		for (j = 0; j < PLAYER_PER_ACCOUNT; ++j)
		{
			if (pkTab->players[j].dwID == player_id)
			{
				...
				if (pt)
				{
					...
					pkTab->players[j].Semeskill		= pt->Semeskill;
					pkTab->players[j].Eemeskill		= pt->Eemeskill;
					pkTab->players[j].titre			= pt->titre;
					pkTab->players[j].cheque		= pt->cheque;
				}
				else
				{
					if (!row[col++])
					...
					pkTab->players[j].bChangeName	= 0;
					pkTab->players[j].Semeskill		= 0;
					pkTab->players[j].Eemeskill		= 0;
					pkTab->players[j].titre			= 0;
					pkTab->players[j].cheque			= 0;
					...
					str_to_number(pkTab->players[j].Semeskill, row[col++]);
					str_to_number(pkTab->players[j].Eemeskill, row[col++]);
					str_to_number(pkTab->players[j].titre, row[col++]);
					str_to_number(pkTab->players[j].cheque, row[col++]);
...
void CClientManager::RESULT_LOGIN(CPeer * peer, SQLMsg * msg)
{
	...
	if (info->account_index == 0)
	{
		...
		else
		{
			...
			if (g_stLocale == "gb2312")
			{
				snprintf(queryStr, sizeof(queryStr), "SELECT id, name, job, level, alignment, st, ht, dx, iq, part_main, part_hair, part_acce, x, y, skill_group, change_name, 7emeSkill, 8emeSkill, cheque, titre FROM player%s WHERE account_id=%u", GetTablePostfix(), info->pAccountTable->id);
			}
			else
			{
				snprintf(queryStr, sizeof(queryStr), "SELECT id, name, job, level, playtime, st, ht, dx, iq, part_main, part_hair, part_acce, x, y, skill_group, change_name, 7emeSkill, 8emeSkill, cheque, titre FROM player%s WHERE account_id=%u", GetTablePostfix(), info->pAccountTable->id);
			}
