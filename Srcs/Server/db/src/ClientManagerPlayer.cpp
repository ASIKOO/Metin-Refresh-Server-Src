...
void CClientManager::QUERY_PLAYER_LOAD(CPeer * peer, DWORD dwHandle, TPlayerLoadPacket * packet)
{
	...
	if ((c = GetPlayerCache(packet->player_id)))
	{
		...
		sys_log(0, "[PLAYER_LOAD] ID %s pid %d gold %lld won %d titre %d", pTab->name, pTab->id, pTab->gold, pTab->cheque, pTab->titre);
		...
		else
		{
			...
			snprintf(szQuery, sizeof(szQuery), 
					...
					"SELECT id,window+0,pos,count,vnum,dwVnum,socket0,socket1,socket2,attrtype0,attrvalue0,attrtype1,attrvalue1,attrtype2,attrvalue2,attrtype3,attrvalue3,attrtype4,attrvalue4,attrtype5,attrvalue5,attrtype6,attrvalue6,applytype0,applyvalue0,applytype1,applyvalue1,applytype2,applyvalue2,applytype3,applyvalue3,applytype4,applyvalue4,applytype5,applyvalue5,applytype6,applyvalue6,applytype7,applyvalue7 "
	...
	else
	{
		...
		snprintf(queryStr, sizeof(queryStr),
				...
				"UNIX_TIMESTAMP(NOW())-UNIX_TIMESTAMP(last_play),horse_skill_point,7emeSkill,8emeSkill,cheque,titre FROM player%s WHERE id=%d",
...
#ifdef ENABLE_SKILL_7ET8_SYSTEM_
	str_to_number(pkTab->Semeskill, row[col++]);
	str_to_number(pkTab->Eemeskill, row[col++]);
#endif
#ifdef ENABLE_CHEQUE_SYSTEM
	str_to_number(pkTab->cheque, row[col++]);
#endif
#ifdef ENABLE_TITRE_SYSTEM
	str_to_number(pkTab->titre, row[col++]);
#endif
