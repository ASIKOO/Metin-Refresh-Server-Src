...
void CClientManager::RESULT_SAFEBOX_LOAD(CPeer * pkPeer, SQLMsg * msg)
{
	...
	if (pi->account_index == 0)
	{
		...
		char szQuery[8192];
		snprintf(szQuery, sizeof(szQuery), "SELECT id, window+0, pos, count, vnum, dwVnum, socket0, socket1, socket2, attrtype0, attrvalue0, attrtype1, attrvalue1, attrtype2, attrvalue2, attrtype3, attrvalue3, attrtype4, attrvalue4, attrtype5, attrvalue5, attrtype6, attrvalue6, applytype0, applyvalue0, applytype1, applyvalue1, applytype2, applyvalue2, applytype3, applyvalue3, applytype4, applyvalue4, applytype5, applyvalue5, applytype6, applyvalue6, applytype7, applyvalue7 FROM item%s WHERE owner_id=%d AND window='%s'", GetTablePostfix(), pi->account_id, pi->ip[0] == 0 ? "SAFEBOX" : "MALL");
...
void CClientManager::QUERY_ITEM_SAVE(CPeer * pkPeer, const char * c_pData)
{
	...
	if (p->window == SAFEBOX || p->window == MALL)
	{
		...
		sys_err("Mise à jour de l'item %d , du personnage %d , type %d, vnum %d", p->id, p->owner, p->window, p->vnum);
		char szQuery1[8192];
		snprintf(szQuery1, sizeof(szQuery1), "DELETE FROM item%s WHERE id = '%u'", GetTablePostfix(), p->id);
		std::unique_ptr<SQLMsg> pkMsg1(CDBManager::instance().DirectQuery(szQuery1));
	
		char szQuery[8192];
		snprintf(szQuery, sizeof(szQuery), 
			"INSERT INTO item%s (id, owner_id, window, pos, count, vnum, dwVnum, socket0, socket1, socket2, "
			...
			"VALUES(%u, %u, %d, %d, %u, %u, %u, %ld, %ld, %ld, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d)",
			GetTablePostfix(),
			p->id,
			p->owner,
			p->window,
